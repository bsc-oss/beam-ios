...
extension PreviewTests {
    ...
    @Test
    func authenticationStartLogo() async throws {
        AppSettings.resetAllSettings() // Ensure this test's previews start with fresh settings.
        for (index, preview) in AuthenticationStartLogo_Previews._allPreviews.enumerated() {
            try await assertSnapshots(matching: preview, step: index)
        }
    }
    ...
    @Test
    func pgEmailInputScreen() async throws {
        AppSettings.resetAllSettings() // Ensure this test's previews start with fresh settings.
        for (index, preview) in PgEmailInputScreen_Previews._allPreviews.enumerated() {
            try await assertSnapshots(matching: preview, step: index)
        }
    }

    @Test
    func pgServiceMessageBanner() async throws {
        AppSettings.resetAllSettings() // Ensure this test's previews start with fresh settings.
        for (index, preview) in PgServiceMessageBanner_Previews._allPreviews.enumerated() {
            try await assertSnapshots(matching: preview, step: index)
        }
    }

    @Test
    func phoneBookConsentSelectionView() async throws {
        AppSettings.resetAllSettings() // Ensure this test's previews start with fresh settings.
        for (index, preview) in PhoneBookConsentSelectionView_Previews._allPreviews.enumerated() {
            try await assertSnapshots(matching: preview, step: index)
        }
    }
