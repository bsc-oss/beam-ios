...
struct AttributedStringBuilderTests {
    ...
    func renderHTMLStringWithLink() throws {
        ...
        #expect(link?.effectiveURL.host == "www.alpha.org")
    }
    ...
    func renderPlainStringWithLink() throws {
        ...
        #expect(link?.effectiveURL.host == "www.matrix.org")
    }
    ...
    func punctuationAtTheEndOfPlainStringLinks() throws {
        ...
        #expect(firstLink?.effectiveURL.absoluteString == "https://www.matrix.org")
        let secondLink = attributedString.runs.last { $0.link != nil }?.link
        #expect(secondLink?.effectiveURL.absoluteString == "https://example.com/(test)")
    }
    ...
    func linkDefaultScheme() throws {
        ...
        #expect(link?.effectiveURL.absoluteString == "https://matrix.org")
    }
    ...
    func mailToLinks() throws {
        ...
        let link = attributedString.runs.first { $0.link != nil }?.link
        #expect(link?.effectiveURL.absoluteString == "mailto:stefan@matrix.org")
    }
    ...
    func renderHTMLStringWithLinkInHeader() throws {
        ...
        #expect(h1AttributedString.runs.first?.link?.effectiveURL.host == "matrix.org")
        #expect(h2AttributedString.runs.first?.link?.effectiveURL.host == "matrix.org")
        #expect(h3AttributedString.runs.first?.link?.effectiveURL.host == "matrix.org")
    }
    ...
    func roomAliasLink() throws {
        ...
        let expectedLink = try #require(URL(string: "https://share.beam.belgium.be/#/\(roomAlias)"), "The expected link should be valid.")
    ...
    func customForegroundColor() throws {
        ...
        for run in attributedString.runs {
            if run.link != nil {
                #expect(run.link?.effectiveURL.host == "www.matrix.org")
                #expect(run.uiKit.foregroundColor == nil)
                foundLink = true
            } else {
                #expect(run.uiKit.foregroundColor == nil)
    ...
    func userIDMentionAtachment() {
        ...
        #expect(attributedStringFromHTML?.attachment != nil)
        #expect(attributedStringFromHTML?.userID == "@test:matrix.org")
        #expect(attributedStringFromHTML?.link?.absoluteString == "https://share.beam.belgium.be/#/@test:matrix.org")
        let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
        #expect(attributedStringFromPlain?.attachment != nil)
        #expect(attributedStringFromPlain?.userID == "@test:matrix.org")
        #expect(attributedStringFromPlain?.link?.absoluteString == "https://share.beam.belgium.be/#/@test:matrix.org")
    }
    ...
    func roomAliasPermalinkMentionAttachment() {
        ...
        #expect(attributedStringFromHTML?.link?.absoluteString == "https://share.beam.belgium.be/#/%23test:matrix.org")
        let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
        #expect(attributedStringFromPlain?.attachment != nil)
        #expect(attributedStringFromHTML?.roomAlias == "#test:matrix.org")
        #expect(attributedStringFromPlain?.link?.absoluteString == "https://share.beam.belgium.be/#/%23test:matrix.org")
    }
    ...
    func eventRoomAliasPermalinkMentionAttachment() {
        ...
        #expect(attributedStringFromHTML?.link?.absoluteString == "https://share.beam.belgium.be/#/%23test:matrix.org/$test")
        let attributedStringFromPlain = attributedStringBuilder.fromPlain(string)
        #expect(attributedStringFromPlain?.attachment != nil)
        #expect(attributedStringFromPlain?.eventOnRoomAlias == .some(.init(alias: "#test:matrix.org", eventID: "$test")))
        #expect(attributedStringFromPlain?.link?.absoluteString == "https://share.beam.belgium.be/#/%23test:matrix.org/$test")
    }
    ...
    func multipleMentions() throws {
        let url = try #require(URL(string: "https://share.beam.belgium.be/#/@test:matrix.org"), "Invalid url")
    ...
    func validLink() throws {
        ...
        let link = try #require(attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
        #expect(!link.requiresConfirmation)
        #expect(link.requiresExternalLinkConfirmation)
        #expect(link.effectiveURL.absoluteString == "https://matrix.org")
    }
    ...
    func validUserIDLink() throws {
        ...
        let link = try #require(attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
        #expect(!link.requiresConfirmation)
        #expect(link.absoluteString == "https://share.beam.belgium.be/#/@alice:matrix.org")
    }
    ...
    func phishingUserIDWithAnotherUserIDPermalink() throws {
        ...
        let link = try #require(attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
        #expect(link.requiresConfirmation)
        #expect(link.confirmationParameters?.internalURL.absoluteString == "https://share.beam.belgium.be/#/@bob:matrix.org")
        #expect(link.confirmationParameters?.displayString == "@alice:matrix.org")
    }
    ...
    func validLinkWithDistractingCharacters() throws {
        ...
        #expect(!link.requiresConfirmation)
        #expect(link.requiresExternalLinkConfirmation)
        #expect(link.effectiveURL.absoluteString == "https://element.io")
    }
    ...
    func phishingMatrixPermalinks() throws {
        ...
        #expect(link.requiresConfirmation)
        #expect(link.confirmationParameters?.internalURL.absoluteString == "https://share.beam.belgium.be/#/%23offensive-room:matrix.org")
        #expect(link.confirmationParameters?.displayString == "https://share.beam.belgium.be/#/#beautiful-room:matrix.org")
    }
    ...
    func validMatrixPermalinks() throws {
        ...
        #expect(!link.requiresConfirmation)
        #expect(link.absoluteString == "https://share.beam.belgium.be/#/%23beautiful-room:matrix.org")
    }
    ...
    func validRoomAliasLink() throws {
        ...
        let link = try #require(attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
        #expect(!link.requiresConfirmation)
        #expect(link.absoluteString == "https://share.beam.belgium.be/#/%23room:matrix.org")
    }
    ...
    func phishingRoomAliasWithAnotherRoomAliasPermalink() throws {
        ...
        let link = try #require(attributedString.runs.first { $0.link != nil }?.link, "Couldn't find the link")
        #expect(link.requiresConfirmation)
        #expect(link.confirmationParameters?.internalURL.absoluteString == "https://share.beam.belgium.be/#/%23another-room:matrix.org")
        #expect(link.confirmationParameters?.displayString == "#room:matrix.org")
    }
    ...
    private func checkLinkIn(attributedString: AttributedString?, expectedLink: String, expectedRuns: Int) throws {
        ...
        for run in attributedString.runs where run.link != nil {
            ...
            #expect(run.link?.effectiveURL.absoluteString == expectedLink)
            return
        }
