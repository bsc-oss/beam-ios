...
final class CreateRoomScreenViewModelTests {
    ...
    @Test
    func defaultState() {
        setup(publicRoomCreationEnabled: true) // PG_CHANGED
    ...
    // PG_CHANGED
    @Test
    func publicRoomCreationDisabledRemovesPublicAccessType() {
        setup(publicRoomCreationEnabled: true)
        // The setup enables publicRoomCreationEnabled, so public should be available
        #expect(context.viewState.availableAccessTypes.contains(.public))
        
        // When flag is disabled, public should not be available
        AppSettings.resetAllSettings()
        let appSettings = AppSettings()
        appSettings.knockingEnabled = true
        appSettings.publicRoomCreationEnabled = false
        let userSession = UserSessionMock(.init(clientProxy: ClientProxyMock(.init(userIDServerName: "matrix.org", userID: "@a:b.com"))))
        let vm = CreateRoomScreenViewModel(isSpace: false,
        ...
        let availableTypes = vm.context.viewState.availableAccessTypes
        #expect(!availableTypes.contains(.public))
        #expect(availableTypes.contains(.private))
    }
    ...
    func createRoomInAnPublicSpaceAvailableTypes() {
        ...
        setup(spacesSelectionMode: .editableSpacesList(preSelectedSpace: space), publicRoomCreationEnabled: true) // PG_CHANGED
    ...
    private func setup(isSpace: Bool = false, spacesSelectionMode: CreateRoomScreenSpaceSelectionMode = .editableSpacesList(preSelectedSpace: nil), publicRoomCreationEnabled: Bool = false) {
        ...
        userSession = UserSessionMock(.init(clientProxy: clientProxy))
        ServiceLocator.shared.settings.knockingEnabled = true
        ServiceLocator.shared.settings.publicRoomCreationEnabled = publicRoomCreationEnabled // PG_CHANGED
