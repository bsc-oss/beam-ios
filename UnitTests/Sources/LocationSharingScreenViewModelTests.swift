...
//
//
// Modified by Belgian Secure Communications for Beam application on 2026-06-07
...
struct LocationSharingScreenViewModelTests {
    ...
    mutating func viewLiveInitialSenderShownCorrectly() {
        ...
        // PG_CHANGED - profiles are resolved from room members, carrying the custom PG fields
        #expect(annotation?.kind == .liveUser(.init(member: RoomMemberProxyMock.mockAlice)))
    }
    ...
    mutating func viewLiveProfilesResolvedFromRoomMembers() async throws {
        ...
        let annotations = context.viewState.annotations
        // PG_CHANGED - profiles are resolved from room members, carrying the custom PG fields
        #expect(annotations.first { $0.id == "@alice:matrix.org" }?.kind == .liveUser(.init(member: RoomMemberProxyMock.mockAlice)))
        #expect(annotations.first { $0.id == "@bob:matrix.org" }?.kind == .liveUser(.init(member: RoomMemberProxyMock.mockBob)))
        #expect(annotations.first { $0.id == "@charlie:matrix.org" }?.kind == .liveUser(.init(member: RoomMemberProxyMock.mockCharlie)))
    }
