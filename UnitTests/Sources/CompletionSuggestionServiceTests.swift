...
final class CompletionSuggestionServiceTests: XCTestCase {
    ...
    func testUserSuggestions() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(), rawSuggestionText: "ali")]
        }
    ...
    func testUserSuggestionsWithEmptyText() async throws {
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)), range: .init(), rawSuggestionText: ""),
                            .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(), rawSuggestionText: ""),
                            .init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL, email: bob.email)), range: .init(), rawSuggestionText: "")]
        }
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)), range: .init(location: 0, length: 1), rawSuggestionText: ""),
                            .init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(location: 0, length: 1), rawSuggestionText: ""),
                            .init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL, email: bob.email)), range: .init(location: 0, length: 1), rawSuggestionText: "")]
        }
    ...
    // PG_CHANGED
    func testUserSuggestionsWithEmail() async throws {
        let alice: RoomMemberProxyMock = .mockAlice
        let bob: RoomMemberProxyMock = .mockBob
        let members: [RoomMemberProxyMock] = [alice, bob, .mockMe]
        let roomProxyMock = JoinedRoomProxyMock(.init(id: "roomID",
        ...
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(), rawSuggestionText: "alice@matr")]
        }
        service.setSuggestionTrigger(.init(type: .user, text: "alice@matr", range: .init()))
        try await deferred.fulfill()
    }
    ...
    func testUserSuggestionInDifferentMessagePositions() async throws {
        ...
        // PG_CHANGED
        var deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(location: 0, length: 3), rawSuggestionText: "al")]
        }
        ...
        // PG_CHANGED
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(location: 5, length: 3), rawSuggestionText: "al")]
        }
    ...
    func testUserSuggestionWithMultipleMentionSymbol() async throws {
        ...
        deffered = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: bob.userID, displayName: bob.displayName, avatarURL: bob.avatarURL, email: bob.email)), range: .init(location: 9, length: 3), rawSuggestionText: "bo")]
        }
    ...
    func testSuggestionsWithMultipleDifferentTriggers() async throws {
        ...
        // PG_CHANGED
        deffered = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .user(.init(id: alice.userID, displayName: alice.displayName, avatarURL: alice.avatarURL, email: alice.email)), range: .init(location: 9, length: 3), rawSuggestionText: "al")]
        }
