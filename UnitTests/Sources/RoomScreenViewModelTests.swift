...
final class RoomScreenViewModelTests {
    ...
    func knockRequestBanner() async throws {
        // PG_CHANGED - KnockRequestProxyMock requires an email
        let roomProxyMock = JoinedRoomProxyMock(.init(knockRequestsState: .loaded([KnockRequestProxyMock(.init(eventID: "1", userID: "@alice:matrix.org", displayName: "Alice", email: "alice@matrix.org", reason: "Hello World!")),
                                                                                   // This one should be filtered
                                                                                   KnockRequestProxyMock(.init(eventID: "2", userID: "@bob:matrix.org", email: "bob@matrix.org", isSeen: true))]),
                                                      joinRule: .knock))
        let viewModel = RoomScreenViewModel(userSession: UserSessionMock(.init()),
                                            roomProxy: roomProxyMock,
                                            initialSelectedPinnedEventID: nil,
        ...
        var deferred = deferFulfillment(viewModel.context.$viewState) { state in
            state.shouldSeeKnockRequests &&
                state.unseenKnockRequests == [.init(displayName: "Alice", email: "alice@matrix.org", avatarURL: nil, userID: "@alice:matrix.org", reason: "Hello World!", eventID: "1")]
        }
        try await deferred.fulfill()
    ...
    func knockRequestBannerMarkAsSeen() async throws {
        ...
        var deferred = deferFulfillment(viewModel.context.$viewState) { state in
            state.shouldSeeKnockRequests &&
                state.unseenKnockRequests == [.init(displayName: "Alice", email: "alice@matrix.org", avatarURL: nil, userID: "@alice:matrix.org", reason: "Hello World!", eventID: "1"),
                                              .init(displayName: nil, email: nil, avatarURL: nil, userID: "@bob:matrix.org", reason: nil, eventID: "2")]
        }
        try await deferred.fulfill()
    ...
    // MARK: - PG_CHANGED - History Sharing Icon Hidden in DMs

    @Test
    func historySharingIconHiddenInDirectRooms() async throws {
        // Given an encrypted DM whose history is shared.
        var configuration = JoinedRoomProxyMockConfiguration(isDirect: true, isEncrypted: true, historyVisibility: .shared)
        let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(RoomInfoProxyMock(configuration))
        let roomProxyMock = JoinedRoomProxyMock(configuration)
        roomProxyMock.underlyingInfoPublisher = infoSubject.asCurrentValuePublisher()
        ...
        // Then the icon should never be shown, whatever the history visibility is.
        let deferred = deferFailure(viewModel.context.$viewState,
                                    timeout: .seconds(1),
                                    message: "The icon should be hidden in a DM") { viewState in
            viewState.roomHistorySharingState != nil
        }
    ...
    // MARK: - PG_CHANGED - Call Button Visibility When Alone in Room
    
    @Test
    func callButtonHiddenWhenUserIsAloneInRoom() {
        // Given a room with only the current user (1 joined member).
        var configuration = JoinedRoomProxyMockConfiguration()
        configuration.members = [.mockMe] // Only one member joined
        let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(RoomInfoProxyMock(configuration))
        let roomProxyMock = JoinedRoomProxyMock(configuration)
        roomProxyMock.underlyingInfoPublisher = infoSubject.asCurrentValuePublisher()
        ...
        let viewModel = RoomScreenViewModel(userSession: UserSessionMock(.init()),
                                            ...
                                            appHooks: AppHooks(),
                                            analyticsService: .mock(settings: appSettings),
                                            userIndicatorController: UserIndicatorControllerMock.default) // PG_CHANGED - ServiceLocator removed upstream
        self.viewModel = viewModel
        
        // Then the user is alone and call button should be hidden.
        #expect(viewModel.state.isUserAloneInRoom)
        #expect(!viewModel.state.shouldShowCallButton)
    }
    
    @Test
    func callButtonShownWhenUserIsNotAloneInRoom() {
        // Given a room with multiple joined members.
        var configuration = JoinedRoomProxyMockConfiguration()
        configuration.members = [.mockMe, .mockAlice] // Two members joined
        let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(RoomInfoProxyMock(configuration))
        let roomProxyMock = JoinedRoomProxyMock(configuration)
        roomProxyMock.underlyingInfoPublisher = infoSubject.asCurrentValuePublisher()
        ...
        // Then the user is not alone and call button should be shown.
        #expect(!viewModel.state.isUserAloneInRoom)
        #expect(viewModel.state.shouldShowCallButton)
    }
    
    @Test
    func callButtonVisibilityUpdatesWhenMembersChange() async throws {
        // Given a room initially with only the current user.
        var configuration = JoinedRoomProxyMockConfiguration()
        configuration.members = [.mockMe]
        let infoSubject = CurrentValueSubject<RoomInfoProxyProtocol, Never>(RoomInfoProxyMock(configuration))
        let roomProxyMock = JoinedRoomProxyMock(configuration)
        roomProxyMock.underlyingInfoPublisher = infoSubject.asCurrentValuePublisher()
        ...
        // Initially the user is alone and call button should be hidden.
        #expect(viewModel.state.isUserAloneInRoom)
        #expect(!viewModel.state.shouldShowCallButton)
        
        // When another member joins the room.
        var deferred = deferFulfillment(viewModel.context.$viewState) { !$0.isUserAloneInRoom }
        configuration.members = [.mockMe, .mockAlice]
        infoSubject.send(RoomInfoProxyMock(configuration))
        try await deferred.fulfill()
        
        // Then call button should become visible.
        #expect(!viewModel.state.isUserAloneInRoom)
        #expect(viewModel.state.shouldShowCallButton)
        
        // When the other member leaves and user is alone again.
        deferred = deferFulfillment(viewModel.context.$viewState) { $0.isUserAloneInRoom }
        configuration.members = [.mockMe]
        infoSubject.send(RoomInfoProxyMock(configuration))
        try await deferred.fulfill()
        
        // Then call button should be hidden again.
        #expect(viewModel.state.isUserAloneInRoom)
        #expect(!viewModel.state.shouldShowCallButton)
    }
}
