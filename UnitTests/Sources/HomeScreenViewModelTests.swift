...
final class HomeScreenViewModelTests {
    ...
    var pgServiceMessageService: PgServiceMessageServiceMock! // PG_CHANGED
    ...
    func acceptSpaceInvite() async throws {
        // PG_CHANGED - spacesEnabled must be true for space invites to work
    ...
    // PG_CHANGED - tests for spacesEnabled feature flag
    
    @Test
    func spacesDisabledFiltersSpacesFromRoomList() {
        appSettings.spacesEnabled = false
        setupViewModel(invites: .spaces)
        
        // Space invites should be filtered out when spacesEnabled is false
        let invites = context.viewState.rooms.invites
        #expect(invites.isEmpty, "Space invites should not appear when spacesEnabled is false")
    }
    
    @Test
    func spacesEnabledShowsSpacesInRoomList() {
        appSettings.spacesEnabled = true
        setupViewModel(invites: .spaces)
        
        // Space invites should be visible when spacesEnabled is true
        let invites = context.viewState.rooms.invites
        #expect(invites.count == 2, "Space invites should appear when spacesEnabled is true")
    }
    
    @Test
    func spacesDisabledBlocksSpaceInviteAcceptance() async throws {
        appSettings.spacesEnabled = false
        setupViewModel(invites: .spaces)
        
        // Force a space invite into the room list to simulate accepting it
        let spaceID = [RoomSummary].mockSpaceInvites[0].id
        
        // Collect actions for a short time — none should be .presentSpace
        var receivedActions = [HomeScreenViewModelAction]()
        let cancellable = viewModel.actions.sink { receivedActions.append($0) }
        
        context.send(viewAction: .acceptInvite(roomIdentifier: spaceID))
        try await Task.sleep(for: .milliseconds(200))
        
        cancellable.cancel()
        let hasSpaceAction = receivedActions.contains { action in
            if case .presentSpace = action { return true }
            return false
        }
        #expect(!hasSpaceAction, "presentSpace should not be emitted when spacesEnabled is false")
    }
    
    @Test
    func spacesDisabledByDefault() {
        #expect(!appSettings.spacesEnabled, "spacesEnabled should default to false")
    }
    
    // PG_CHANGED - argus entrypoint moved to a launcher tab; its tests live in UserSessionFlowCoordinatorTests
    ...
    // PG_CHANGED
    @Test
    func showsInitialServiceMessage() {
        let serviceMessage = PgServiceMessageDisplay(id: 11,
                                                     isCritical: true,
                                                     allowDismiss: true,
                                                     title: "Maintenance",
                                                     body: "Server notice")
        setupViewModel(serviceMessage: serviceMessage)

        #expect(context.viewState.serviceMessage == serviceMessage)
        #expect(context.viewState.shouldShowServiceMessage)
        #expect(context.viewState.shouldShowBanner)
    }

    @Test
    func dismissServiceMessageCallsService() {
        setupViewModel(serviceMessage: .init(id: 12,
                                             isCritical: false,
                                             allowDismiss: true,
                                             title: nil,
                                             body: "Dismiss me"))

        context.send(viewAction: .dismissServiceMessage(id: 12))

        #expect(pgServiceMessageService.dismissMessageIDCalled)
        #expect(pgServiceMessageService.dismissMessageIDReceivedMessageID == 12)
    }

    @Test
    func hidesServiceMessageWhileSearching() {
        setupViewModel(serviceMessage: .init(id: 13,
                                             isCritical: false,
                                             allowDismiss: true,
                                             title: nil,
                                             body: "Search-hidden"))
        ...
        context.isSearchFieldFocused = true
        context.searchQuery = "beam"

        #expect(!context.viewState.shouldShowServiceMessage)
    }

    @Test
    func hidesServiceMessageWhileFiltering() {
        setupViewModel(serviceMessage: .init(id: 14,
                                             isCritical: false,
                                             allowDismiss: true,
                                             title: nil,
                                             body: "Filter-hidden"))
    ...
    private func setupViewModel(securityStatePublisher: CurrentValuePublisher<SessionSecurityState, Never>? = nil,
                                ...
                                serviceMessage: PgServiceMessageDisplay? = nil) {
        ...
        // PG_CHANGED
        pgServiceMessageService = PgServiceMessageServiceMock()
        pgServiceMessageService.underlyingCurrentMessagePublisher = CurrentValueSubject<PgServiceMessageDisplay?, Never>(serviceMessage).asCurrentValuePublisher()
        ...
        viewModel = HomeScreenViewModel(userSession: userSession,
                                        ...
                                        pgServiceMessageService: pgServiceMessageService) // PG_CHANGED
    }
}
