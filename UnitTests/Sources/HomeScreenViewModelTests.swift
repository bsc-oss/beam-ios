...
class HomeScreenViewModelTests: XCTestCase {
    ...
    func testSpacesDisabledFiltersSpacesFromRoomList() {
        ...
        XCTAssertTrue(invites.isEmpty, "Space invites should not appear when spacesEnabled is false")
    }
    
    func testSpacesEnabledShowsSpacesInRoomList() {
        appSettings.spacesEnabled = true
        setupViewModel(invites: .spaces)
        
        // Space invites should be visible when spacesEnabled is true
        let invites = context.viewState.rooms.invites
        XCTAssertEqual(invites.count, 2, "Space invites should appear when spacesEnabled is true")
    }
    
    func testSpacesDisabledBlocksSpaceInviteAcceptance() async throws {
        appSettings.spacesEnabled = false
        setupViewModel(invites: .spaces)
        ...
        XCTAssertFalse(hasSpaceAction, "presentSpace should not be emitted when spacesEnabled is false")
    }
    
    func testSpacesDisabledByDefault() {
        XCTAssertFalse(appSettings.spacesEnabled, "spacesEnabled should default to false")
    }
