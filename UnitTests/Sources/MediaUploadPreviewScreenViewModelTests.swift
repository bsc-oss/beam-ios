...
final class MediaUploadPreviewScreenViewModelTests {
    ...
    // PG_CHANGED
    @Test
    func blockedFilesMixedBatchShowsAlertAndContinuesWithAllowedFiles() async throws {
        ...
        #expect(context.blockedFilesAlertInfo == nil)
        ...
        #expect(context.blockedFilesAlertInfo?.blockedFilenames.count == 1)
        #expect(context.blockedFilesAlertInfo?.blockedFilenames.first?.hasSuffix("script.bat") ?? false)
        #expect(context.blockedFilesAlertInfo?.canContinue == true)
        ...
        try await deferredDismiss.fulfill()
        #expect(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount == 1)
        #expect(context.blockedFilesAlertInfo == nil)
    }

    // PG_CHANGED
    @Test
    func allBlockedFilesShowsAlertAndDoesNotSend() async throws {
        ...
        try await deferredBlockedAlert.fulfill()
        #expect(context.blockedFilesAlertInfo?.blockedFilenames.count == 1)
        #expect(context.blockedFilesAlertInfo?.blockedFilenames.first?.hasSuffix("malware.exe") ?? false)
        #expect(context.blockedFilesAlertInfo?.canContinue == false)
        #expect(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount == 0)
    ...
    // PG_CHANGED
    @Test
    func dismissBlockedFilesAlertClearsAlertAndDoesNotSend() async throws {
        let blockedURL = try createTemporaryFile(named: "script.bat")
