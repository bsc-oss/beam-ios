...
class MediaUploadPreviewScreenViewModelTests: XCTestCase {
    ...
    // PG_CHANGED
    func testBlockedFilesMixedBatchShowsAlertAndContinuesWithAllowedFiles() async throws {
        let blockedURL = try createTemporaryFile(named: "script.bat")

        // Given an upload screen with one allowed and one blocked file.
        setUpViewModel(urls: [fileURL, blockedURL], expectedCaption: nil)
        XCTAssertNil(context.blockedFilesAlertInfo)

        // When attempting to send.
        let deferredBlockedAlert = deferFulfillment(context.observe(\.viewState.bindings.blockedFilesAlertInfo)) { $0 != nil }
        context.send(viewAction: .send)

        // Then a blocked-files sheet is presented and no upload is sent yet.
        try await deferredBlockedAlert.fulfill()
        XCTAssertEqual(context.blockedFilesAlertInfo?.blockedFilenames.count, 1)
        XCTAssertTrue(context.blockedFilesAlertInfo?.blockedFilenames.first?.hasSuffix("script.bat") ?? false)
        XCTAssertEqual(context.blockedFilesAlertInfo?.canContinue, true)
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 0)

        // When the user continues.
        let deferredDismiss = deferFulfillment(viewModel.actions) { $0 == .dismiss }
        context.send(viewAction: .confirmBlockedFilesAlert)

        // Then only the allowed file is uploaded and the screen is dismissed.
        try await deferredDismiss.fulfill()
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 1)
        XCTAssertNil(context.blockedFilesAlertInfo)
    }

    // PG_CHANGED
    func testAllBlockedFilesShowsAlertAndDoesNotSend() async throws {
        let blockedURL = try createTemporaryFile(named: "malware.exe")

        // Given an upload screen where all files are blocked.
        setUpViewModel(urls: [blockedURL], expectedCaption: nil)
        XCTAssertNil(context.blockedFilesAlertInfo)
        ...
        // Then a blocked-files sheet is presented with no continue option.
        try await deferredBlockedAlert.fulfill()
        XCTAssertEqual(context.blockedFilesAlertInfo?.blockedFilenames.count, 1)
        XCTAssertTrue(context.blockedFilesAlertInfo?.blockedFilenames.first?.hasSuffix("malware.exe") ?? false)
        XCTAssertEqual(context.blockedFilesAlertInfo?.canContinue, false)
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 0)

        // When the user acknowledges the sheet.
        let deferredDismiss = deferFulfillment(viewModel.actions) { $0 == .dismiss }
        context.send(viewAction: .confirmBlockedFilesAlert)

        // Then no upload is sent and the preview is dismissed.
        try await deferredDismiss.fulfill()
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 0)
        XCTAssertNil(context.blockedFilesAlertInfo)
    }
    ...
    // PG_CHANGED
    func testDismissBlockedFilesAlertClearsAlertAndDoesNotSend() async throws {
        let blockedURL = try createTemporaryFile(named: "script.bat")
        ...
        // Then a blocked-files sheet is presented and nothing has been uploaded.
        try await deferredBlockedAlert.fulfill()
        XCTAssertEqual(context.blockedFilesAlertInfo?.blockedFilenames.count, 1)
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 0)

        // When the user dismisses the sheet.
        let deferredFailure = deferFailure(viewModel.actions,
                                           timeout: 1,
                                           message: "Dismissing the blocked-files sheet should not dismiss the screen.") { $0 == .dismiss }
        context.send(viewAction: .dismissBlockedFilesAlert)

        // Then the alert is cleared and no upload is sent.
        try await deferredFailure.fulfill()
        XCTAssertNil(context.blockedFilesAlertInfo)
        XCTAssertEqual(timelineProxy.sendFileUrlFileInfoCaptionRequestHandleCallsCount, 0)
    }
    ...
    // PG_CHANGED
    private func createTemporaryFile(named filename: String) throws -> URL {
