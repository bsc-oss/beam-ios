...
final class AuthenticationStartScreenViewModelTests {
    ...
    var pgServiceMessageSubject: CurrentValueSubject<PgServiceMessageDisplay?, Never>!
    ...
    // PG_CHANGED
    @Test
    func showsInitialServiceMessage() async throws {
        let serviceMessage = PgServiceMessageDisplay(id: 7,
                                                     isCritical: true,
                                                     allowDismiss: false,
                                                     title: "Maintenance",
                                                     body: "Scheduled maintenance")
        await setupViewModel(serviceMessage: serviceMessage)

        #expect(context.viewState.serviceMessage == serviceMessage)
    }

    @Test
    func updatesServiceMessageFromPublisher() async throws {
        await setupViewModel()
        #expect(context.viewState.serviceMessage == nil)

        let serviceMessage = PgServiceMessageDisplay(id: 8,
                                                     isCritical: false,
                                                     allowDismiss: false,
                                                     title: "Notice",
                                                     body: "Something changed")

        let deferred = deferFulfillment(context.observe(\.viewState.serviceMessage)) { $0 == serviceMessage }
        pgServiceMessageSubject.send(serviceMessage)
        try await deferred.fulfill()
    }
    ...
    private func setupViewModel(classicAppAccount: ClassicAppAccount? = nil,
                                ...
                                // PG_CHANGED - inject the PG service message
                                serviceMessage: PgServiceMessageDisplay? = nil) async {
        ...
        pgServiceMessageSubject = CurrentValueSubject<PgServiceMessageDisplay?, Never>(serviceMessage)
        pgServiceMessageService.underlyingCurrentMessagePublisher = pgServiceMessageSubject.asCurrentValuePublisher()
