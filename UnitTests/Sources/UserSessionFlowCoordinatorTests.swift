...
class UserSessionFlowCoordinatorTests: XCTestCase {
    ...
    func testSpacesDisabledHidesSpacesTab() {
        // spacesEnabled defaults to false, so only the chats tab should be present
        let tabTags = tabCoordinator?.tabTags ?? []
        XCTAssertTrue(tabTags.contains(.chats), "The chats tab should be shown")
        XCTAssertFalse(tabTags.contains(.spaces), "The spaces tab should not be shown when spacesEnabled is false")
    }

    // PG_CHANGED
    func testSpacesEnabledShowsSpacesTab() async throws {
        ...
        let tabTags = tabCoordinator?.tabTags ?? []
        XCTAssertTrue(tabTags.contains(.chats), "The chats tab should be shown")
        XCTAssertTrue(tabTags.contains(.spaces), "The spaces tab should be shown when spacesEnabled is true")
