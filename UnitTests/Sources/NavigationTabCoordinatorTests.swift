...
struct NavigationTabCoordinatorTests {
    ...
    // PG_CHANGED - re-running setTabs must reuse the modules of unchanged coordinators (so their
    // navigation state survives) and only start/tear down the tabs that actually changed.
    @Test
    mutating func reusesModulesForUnchangedCoordinators() async {
        let chatsCoordinator = SomeTestCoordinator()
        let spacesCoordinator = SomeTestCoordinator()

        await waitForConfirmation("Reused tabs should not be torn down",
                                  expectedCount: 0,
                                  timeout: .seconds(1)) { confirm in
            navigationTabCoordinator.setTabs([
            ...
            // Re-run with the same coordinators plus a new tab appended: chats/spaces are reused,
            // so their dismissal callbacks must not fire.
            let argusCoordinator = SomeTestCoordinator()
            navigationTabCoordinator.setTabs([
                .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)),
                .init(coordinator: spacesCoordinator, details: .init(tag: .spaces, title: "Spaces", icon: \.space, selectedIcon: \.spaceSolid)),
                .init(coordinator: argusCoordinator, details: .init(tag: .tab, title: "Argus", icon: \.help, selectedIcon: \.helpSolid))
            ])

            assertCoordinatorsEqual(navigationTabCoordinator.tabCoordinators, [chatsCoordinator, spacesCoordinator, argusCoordinator])
        }
    }

    // PG_CHANGED - the selection must be preserved across setTabs when its tab is still present.
    @Test
    mutating func preservesSelectionWhenTabsChange() {
        let chatsCoordinator = SomeTestCoordinator()
        let spacesCoordinator = SomeTestCoordinator()
        navigationTabCoordinator.setTabs([
            .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)),
        ...
        #expect(navigationTabCoordinator.selectedTab == .chats)

        // Select spaces, then append a tab: selection must be preserved.
        navigationTabCoordinator.selectedTab = .spaces
        let argusCoordinator = SomeTestCoordinator()
        navigationTabCoordinator.setTabs([
            .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)),
            .init(coordinator: spacesCoordinator, details: .init(tag: .spaces, title: "Spaces", icon: \.space, selectedIcon: \.spaceSolid)),
        ...
        #expect(navigationTabCoordinator.selectedTab == .spaces)

        // Removing the selected tab falls back to the first.
        navigationTabCoordinator.setTabs([
            .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid))
        ])
        #expect(navigationTabCoordinator.selectedTab == .chats)
    ...
    // PG_CHANGED - selecting a normal tab through the selection binding switches the displayed tab.
    @Test
    mutating func selectingNormalTabUpdatesSelection() {
        let chatsCoordinator = SomeTestCoordinator()
        let spacesCoordinator = SomeTestCoordinator()
        navigationTabCoordinator.setTabs([
            .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)),
        ...
        navigationTabCoordinator.selectTab(.spaces)
        #expect(navigationTabCoordinator.selectedTab == .spaces)
    }

    // PG_CHANGED - selecting a launcher tab (whose details carry an `onSelect`) runs that action and leaves
    // the current selection untouched, instead of switching to the launcher tab's placeholder content.
    @Test
    mutating func selectingLauncherTabRunsActionWithoutSwitching() {
        let chatsCoordinator = SomeTestCoordinator()
        let argusCoordinator = SomeTestCoordinator()

        var launcherActionCount = 0
        let argusDetails = NavigationTabCoordinator<TestTab>.TabDetails(tag: .spaces, title: "Argus", icon: \.help, selectedIcon: \.helpSolid)
        argusDetails.onSelect = { launcherActionCount += 1 }

        navigationTabCoordinator.setTabs([
            .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)),
            .init(coordinator: argusCoordinator, details: argusDetails)
        ])
        #expect(navigationTabCoordinator.selectedTab == .chats)

        navigationTabCoordinator.selectTab(.spaces)

        // The launcher action ran, and the displayed tab stayed on chats.
        #expect(launcherActionCount == 1)
        #expect(navigationTabCoordinator.selectedTab == .chats)
    }
