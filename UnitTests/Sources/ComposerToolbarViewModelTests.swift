...
class ComposerToolbarViewModelTests: XCTestCase {
    ...
    func testSuggestions() {
        // PG_CHANGED
        let suggestions: [SuggestionItem] = [.init(suggestionType: .user(.init(id: "@user_mention_1:matrix.org", displayName: "User 1", avatarURL: nil, email: "user_mention_1@matrix.org")), range: .init(), rawSuggestionText: ""),
                                             .init(suggestionType: .user(.init(id: "@user_mention_2:matrix.org", displayName: "User 2", avatarURL: nil, email: "user_mention_2@matrix.org")), range: .init(), rawSuggestionText: "")]
    ...
    func testSelectedUserSuggestion() {
        // PG_CHANGED
        let suggestion = SuggestionItem(suggestionType: .user(.init(id: "@test:matrix.org", displayName: "Test", avatarURL: nil, email: "test@matrix.org")), range: .init(), rawSuggestionText: "")
        ...
        XCTAssertEqual(wysiwygViewModel.content.html, "<a href=\"https://share.beam.belgium.be/#/@test:matrix.org\">@test:matrix.org</a> ")
    }
    ...
    func testSelectedRoomSuggestion() {
        ...
        XCTAssertEqual(wysiwygViewModel.content.html, "<a href=\"https://share.beam.belgium.be/#/%23room-alias:matrix.org\">#room-alias:matrix.org</a> ")
    }
    ...
    func testUserMentionPillInRTE() async {
        ...
        // PG_CHANGED
        let suggestion = SuggestionItem(suggestionType: .user(.init(id: userID, displayName: "Test", avatarURL: nil, email: "test@matrix.org")), range: .init(), rawSuggestionText: "")
        viewModel.context.send(viewAction: .selectedSuggestion(suggestion))
    ...
    func testIntentionalMentions() async throws {
        wysiwygViewModel.setHtmlContent(
            """
            <p>Hello @room \
            and especially hello to <a href=\"https://share.beam.belgium.be/#/@test:matrix.org\">Test</a></p>
            """
        )
    ...
    func testRestoreUserMentionInPlainText() async throws {
        viewModel.context.composerFormattingEnabled = false
        let text = "Hello [TestName](https://share.beam.belgium.be/#/@test:matrix.org)!"
        ...
        let deferred = deferFulfillment(viewModel.actions) { action in
            ...
            case let .sendMessage(plainText, _, _, intentionalMentions):
                ...
                return plainText == "Hello [@test:matrix.org](https://share.beam.belgium.be/#/@test:matrix.org)!" &&
    ...
    func testRestoreMixedMentionsInPlainText() async throws {
        viewModel.context.composerFormattingEnabled = false
        let text = "Hello [User1](https://share.beam.belgium.be/#/@user1:matrix.org), [User2](https://share.beam.belgium.be/#/@user2:matrix.org) and @room"
        viewModel.process(timelineAction: .setText(plainText: text, htmlText: nil))
        ...
        let deferred = deferFulfillment(viewModel.actions) { action in
            ...
            case let .sendMessage(plainText, _, _, intentionalMentions):
                // As of right now the markdown loses the display name when restored
                return plainText == "Hello [@user1:matrix.org](https://share.beam.belgium.be/#/@user1:matrix.org), [@user2:matrix.org](https://share.beam.belgium.be/#/@user2:matrix.org) and @room" &&
    ...
    func testRestoreAmbiguousMention() async throws {
        viewModel.context.composerFormattingEnabled = false
        let text = "Hello [User1](https://share.beam.belgium.be/#/@roomuser:matrix.org)"
        viewModel.process(timelineAction: .setText(plainText: text, htmlText: nil))
        ...
        let deferred = deferFulfillment(viewModel.actions) { action in
            ...
            case let .sendMessage(plainText, _, _, intentionalMentions):
                // As of right now the markdown loses the display name when restored
                return plainText == "Hello [@roomuser:matrix.org](https://share.beam.belgium.be/#/@roomuser:matrix.org)" &&
