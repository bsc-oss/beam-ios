...
final class PgEmailInputScreenViewModelTests: XCTestCase {
    ...
    var context: PgEmailInputScreenViewModel.Context { viewModel.context }
    ...
    func testSuccessfulOIDCFlow() async throws {
        setupViewModel(emailValidationResult: .success(.init(url: "domain.be")), supportsOIDC: true)
        context.send(viewAction: .updateWindow(UIWindow()))
        
        viewModel.state.bindings.email = "firstname.lastname@domain.be"
        
        let deferred = deferFulfillment(viewModel.actions) {
            if case .continueWithOIDC = $0 { return true }
            return false
        }
        context.send(viewAction: .confirm)
        try await deferred.fulfill()
        ...
        XCTAssertEqual(client.urlForOidcOidcConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?.loginHint, "firstname.lastname@domain.be")
    }
    ...
    func testIsLoadingSetDuringConfirm() async throws {
        ...
        // isLoading stays true after OIDC action is sent (until clearPendingOIDC is called)
    ...
    func testClearPendingOIDCResetsIsLoading() async throws {
        ...
        // Simulate coordinator calling clearPendingOIDC after OIDC cancel
    ...
    private func assertEmailIsTrimmed(input: String, file: StaticString = #file, line: UInt = #line) async throws {
        ...
        XCTAssertEqual(client.urlForOidcOidcConfigurationPromptLoginHintDeviceIdAdditionalScopesReceivedArguments?.loginHint, expected, "OIDC login hint received untrimmed email", file: file, line: line)
    }
    ...
    private func setupViewModel(emailValidationResult: Result<PgEmailValidationResponse, PgEmailValidationError>,
        ...
        client = ClientSDKMock(configuration: .init(oidcLoginURL: supportsOIDC ? "https://account.domain.be/authorize" : nil,
...
private extension PgEmailInputScreenViewModelAction {
    var isContinueWithOIDC: Bool {
        if case .continueWithOIDC = self { return true }
        return false
    }
}
