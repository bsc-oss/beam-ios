    ...
    private struct ThreadView: View {
        ...
        var body: some View {
            HStack(spacing: 4) {
                ...
                // PG_CHANGED
                Text((sender?.disambiguatedDisplayName ?? sender?.email) ?? senderID)
                    .font(.compound.bodyXSSemibold)
                    .foregroundColor(.compound.textPrimary)
                    // PG_CHANGED
                    .accessibilityLabel(L10n.commonInReplyTo((sender?.disambiguatedDisplayName ?? sender?.email) ?? senderID))
...
struct TimelineThreadSummaryView_Previews: PreviewProvider, TestablePreview {
    ...
    static var previewItems: [TimelineThreadSummaryView] {
        [
            ...
            // PG_CHANGED - no display name and with email
            TimelineThreadSummaryView(threadSummary: .loaded(senderID: "@alice:matrix.org",
                                                             sender: .init(id: "@alice:matrix.org", displayName: nil, email: "alice@matrix.org"),
                                                             latestEventContent: .message(.notice(.init(body: "Hello world"))),
                                                             numberOfReplies: 42))
        ]
    }
