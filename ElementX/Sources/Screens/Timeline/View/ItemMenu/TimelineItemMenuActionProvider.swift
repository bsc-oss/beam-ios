...
struct TimelineItemMenuActionProvider {
    ...
    func makeActions() -> TimelineItemMenuActions? {
        ...
        // PG_CHANGED - allows copyPermalink TimelineItemMenuAction only on dev builds
        if item.isRemoteMessage, isCopyPermalinkEnabled {
        ...
        if item.isCopyable {
            ...
            // PG_CHANGED - puts translate TimelineItemMenuAction behind a feature flag (disabled by default)
            if isTranslateEnabled, !ProcessInfo.processInfo.isiOSAppOnMac {
        ...
        // PG_CHANGED - allows report TimelineItemMenuAction only on dev builds
        if !item.isOutgoing, isReportEnabled {
    ...
    private func makeEncryptedItemActions(_ encryptedItem: EncryptedRoomTimelineItem) -> TimelineItemMenuActions? {
        // PG_CHANGED - allows copyPermalink TimelineItemMenuAction only on dev builds
        var actions: [TimelineItemMenuAction] = isCopyPermalinkEnabled ? [.copyPermalink] : []
