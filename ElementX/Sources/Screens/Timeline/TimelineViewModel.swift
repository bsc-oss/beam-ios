...
class TimelineViewModel: TimelineViewModelType, TimelineViewModelProtocol {
    ...
    private func updateMembers(_ members: [RoomMemberProxyProtocol]) {
        state.members = members.reduce(into: [String: RoomMemberState]()) { dictionary, member in
            // PG_CHANGED
            dictionary[member.userID] = RoomMemberState(displayName: member.displayName, email: member.email, avatarURL: member.avatarURL)
    ...
    private func setupAppSettingsSubscriptions() {
        ...
        appSettings.$copyPermalinkEnabled
            .weakAssign(to: \.state.isCopyPermalinkEnabled, on: self)
            .store(in: &cancellables)
        
        // PG_CHANGED - allows report TimelineItemMenuAction only on dev builds
        appSettings.$reportEnabled
            .weakAssign(to: \.state.isReportEnabled, on: self)
            .store(in: &cancellables)
        
        // PG_CHANGED - puts translate TimelineItemMenuAction behind a feature flag (disabled by default)
        appSettings.$translateEnabled
            .weakAssign(to: \.state.isTranslateEnabled, on: self)
            .store(in: &cancellables)
