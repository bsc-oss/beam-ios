...
struct SettingsScreen: View {
    ...
    var body: some View {
        Form {
            ...
            // PG_CHANGED - interverting the order of the two below sections (same behaviour as before 26.05.3 upstream)
        ...
        Section {
            ListRow(kind: .custom {
                Button {
                    ...
                    HStack(spacing: 12) {
                        LoadableAvatarImage(url: context.viewState.userProfileProxy?.avatarURL,
                                            name: context.viewState.userProfileProxy?.displayName,
                                            email: context.viewState.userProfileProxy?.email,
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(context.viewState.userProfileProxy?.primaryInfo ?? "")
                                .font(.compound.headingMD)
                                .foregroundColor(.compound.textPrimary)
                            Text(context.viewState.userProfileProxy?.secondaryInfo ?? "")
    ...
    private var manageAccountSection: some View {
        Section {
            ...
            // PG_CHANGED
            ListRow(label: .default(title: L10n.pgScreenSettingQrLogin,
                                    icon: \.qrCode),
                    kind: .navigationLink {
                        context.send(viewAction: .loginWithQR)
                    })
                    .accessibilityIdentifier(A11yIdentifiers.settingsScreen.qrLogin)
    ...
    private var generalSection: some View {
        Section {
            ...
            // PG_CHANGED - labs entry removed (threads moved to developer options)
    ...
    private var signOutSection: some View {
        Section {
            // PG_CHANGED: keep the sign-out icon instead of upstream's close icon (26.05.3)
