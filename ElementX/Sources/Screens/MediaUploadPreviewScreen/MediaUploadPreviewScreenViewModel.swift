...
class MediaUploadPreviewScreenViewModel: MediaUploadPreviewScreenViewModelType, MediaUploadPreviewScreenViewModelProtocol {
    ...
    // PG_CHANGED
    private var pendingAllowedMediaInfos: [MediaInfo] = []
    // PG_CHANGED
    ...
    override func process(viewAction: MediaUploadPreviewScreenViewAction) {
        ...
        case .send:
            ...
            Task {
                ...
                case .success(let mediaInfos):
                    // PG_CHANGED
                    let uploadCandidates = mediaInfos.enumerated().map { index, mediaInfo in
                        /*
                           Use the original user-selected URL to preserve the real filename and extension
                           for accurate blocklist checking, since preprocessing may rename/convert files
                           (e.g. .heic → .jpg). Fall back to the processed URL if the index is out of
                           bounds, which can happen if preprocessing changes the number of items.
                         */
                        let originalURL = mediaURLs.indices.contains(index) ? mediaURLs[index] : mediaInfo.url
                        let originalFilename = originalURL.lastPathComponent

                        let isBlocked = FileTypeBlocklist.isBlocked(mimeType: mediaInfo.mimeType, filename: originalFilename)
                        return (mediaInfo: mediaInfo, originalFilename: originalFilename, isBlocked: isBlocked)
                    }

                    let blockedFilenames = uploadCandidates
                        .filter(\.isBlocked)
                        .map(\.originalFilename)

                    let allowedMediaInfos = uploadCandidates
                        .filter { !$0.isBlocked }
                        .map(\.mediaInfo)

                    if !blockedFilenames.isEmpty {
                        pendingAllowedMediaInfos = allowedMediaInfos
                        pendingCaption = caption
                        state.bindings.blockedFilesAlertInfo = .init(blockedFilenames: blockedFilenames,
                                                                     canContinue: !allowedMediaInfos.isEmpty)
                        return
                    }

                    caption = await sendAttachments(mediaInfos, caption: caption)
        ...
        case .confirmBlockedFilesAlert:
            let mediaInfos = pendingAllowedMediaInfos
            pendingAllowedMediaInfos.removeAll()
            state.bindings.blockedFilesAlertInfo = nil
            ...
            Task {
                ...
                await sendAttachments(mediaInfos, caption: pendingCaption)
                pendingCaption = nil
                actionsSubject.send(.dismiss)
            }
        // PG_CHANGED
    ...
    // PG_CHANGED: original Element code put in a function.
    @discardableResult
    private func sendAttachments(_ mediaInfos: [MediaInfo], caption: String?) async -> String? {
        var caption = caption
