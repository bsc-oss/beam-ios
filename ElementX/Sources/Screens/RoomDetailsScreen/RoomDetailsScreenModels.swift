...
struct RoomDetailsScreenViewState: BindableState {
    ...
    // PG_CHANGED - kept knockingEnabled (upstream removed it); still gates the knocking-requests entry
    var knockingEnabled = false
    // PG_CHANGED
    var isShareRoomEnabled = false

    var canSeeKnockingRequests: Bool {
        // PG_CHANGED - keep gating knocking requests behind knockingEnabled (upstream made it always-on)
    ...
    // PG_CHANGED - hide call button when user is alone in room (mirrors RoomScreen.shouldShowCallButton)
    var isUserAloneInRoom: Bool {
        joinedMembersCount <= 1
    }

    var shortcuts: [RoomDetailsScreenViewShortcut] {
        var shortcuts: [RoomDetailsScreenViewShortcut] = [.mute]
        // PG_CHANGED - also hide the call shortcut when the user is alone in the room (normally a DM whose recipient hasn't joined)
        if !ProcessInfo.processInfo.isiOSAppOnMac, canJoinCall, !isUserAloneInRoom {
            // PG_CHANGED - always show voice call for DMs & rooms ; video call only for DMs
