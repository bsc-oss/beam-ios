...
class CallScreenViewModel: CallScreenViewModelType, CallScreenViewModelProtocol {
    ...
    // PG_CHANGED - sets default audio output to earpiece
    private var isDefaultOutputDeviceSet = false
    ...
    private func updateOutputsListOnWeb() async {
        ...
        var javaScript = "window.controls.setAvailableOutputDevices([\(deviceList)]);"
        
        // PG_CHANGED - sets default audio output to earpiece
        if !isDefaultOutputDeviceSet {
            isDefaultOutputDeviceSet = true
            javaScript += "window.controls.setAudioDevice('\(CallScreenViewModel.earpieceID)');"
        }
