...
class RoomMemberDetailsScreenViewModel: RoomMemberDetailsScreenViewModelType, RoomMemberDetailsScreenViewModelProtocol {
    ...
    init(userID: String,
        ...
        // PG_CHANGED - read shareProfileEnabled from the injected appSettings (upstream removed ServiceLocator)
        appSettings.$shareProfileEnabled
            .weakAssign(to: \.state.isShareProfileEnabled, on: self)
    ...
    private func openDirectChat() {
        ...
        case .success(let roomID):
            if let roomID {
                ...
                // PG_CHANGED - thread the member's email into the invited user's profile
                state.bindings.inviteConfirmationUser = .init(user: .init(userID: roomMemberProxy.userID, displayName: roomMemberProxy.displayName, avatarURL: roomMemberProxy.avatarURL, email: roomMemberProxy.email), isUnknown: false)
            }
        case .failure:
