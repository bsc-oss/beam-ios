...
class RoomMemberDetailsScreenViewModel: RoomMemberDetailsScreenViewModelType, RoomMemberDetailsScreenViewModelProtocol {
    ...
    init(userID: String,
        ...
        // PG_CHANGED
        ServiceLocator.shared.settings.$shareProfileEnabled
    ...
    private func openDirectChat() {
        ...
        case .success(let roomID):
            if let roomID {
                ...
                // PG_CHANGED
                state.bindings.inviteConfirmationUser = .init(userID: roomMemberProxy.userID, displayName: roomMemberProxy.displayName, avatarURL: roomMemberProxy.avatarURL, email: roomMemberProxy.email)
            }
        case .failure:
