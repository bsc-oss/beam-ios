...
struct RoomMemberDetailsScreen: View {
    ...
    private var headerSection: some View {
        if let memberDetails = context.viewState.memberDetails {
            AvatarHeaderView(member: memberDetails,
                ...
                VStack(spacing: 24) {
                    // PG_CHANGED
                    PgCustomProfileFieldsView(email: memberDetails.email,
    ...
    private var userIDSection: some View {
        if let memberDetails = context.viewState.memberDetails {
            CopyTextButton(content: memberDetails.userID, font: .compound.bodyMD, iconSize: .xSmall)
        }
    }
    ...
    private var withdrawVerificationSection: some View {
        VStack(spacing: 16) {
            if let memberDetails = context.viewState.memberDetails {
                // PG_CHANGED
                Text(L10n.cryptoIdentityChangeProfilePinViolation(memberDetails.primaryInfo))
    ...
    private var otherUserFooter: some View {
        HStack(spacing: 8) {
            ...
            // PG_CHANGED
            if context.viewState.isShareProfileEnabled, let permalink = context.viewState.memberDetails?.permalink {
...
struct RoomMemberDetailsScreen_Previews: PreviewProvider, TestablePreview {
    ...
    static var previews: some View {
        ...
        RoomMemberDetailsScreen(context: otherUserViewModel.context)
            // PG_CHANGED - also wait for the async identity load to settle (verificationState) so the
            // "Verify user" row renders deterministically; without it the snapshot races. Upstream omits this.
            .snapshotPreferences(expect: otherUserViewModel.context.$viewState.map { state in
                state.memberDetails?.role == .user && state.dmRoomID != nil && state.verificationState == .notVerified
            })
