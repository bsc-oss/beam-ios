...
// PG_CHANGED: Customize QL, show warning before sharing/saving. Remove extra share option from the title menu, fix info button state (bug from upstream).
class TimelineMediaPreviewController: PgPreviewController {
    ...
    private var pageScrollView: UIScrollView? { view.firstSubview(of: UIScrollView.self) }
    ...
    override var currentFileURL: URL? {
        guard case let .media(mediaItem) = context.viewState.currentItem else { return nil }
        return mediaItem.fileHandle?.url
    }
    
    override var currentFileMimeType: String? {
        guard case let .media(mediaItem) = context.viewState.currentItem else { return nil }
    ...
    // MARK: - PgPreviewController hooks

    override func configureNavigationBar() {
        guard let navBar = navigationBar else { return }

        if headerHostingController.view.superview !== navBar {
            let headerView = headerHostingController.view!
            navBar.addSubview(headerView)
            headerView.translatesAutoresizingMaskIntoConstraints = false
            NSLayoutConstraint.activate([
                headerView.centerXAnchor.constraint(equalTo: navBar.centerXAnchor),
                headerView.centerYAnchor.constraint(equalTo: navBar.centerYAnchor)
            ])
        }
    }

    override func updateLeftBarButton() {
        guard let topItem = navigationBar?.topItem else { return }
        if topItem.leftBarButtonItem?.customView !== detailsButtonHostingController.view {
            let button = UIBarButtonItem(customView: detailsButtonHostingController.view)
            topItem.leftBarButtonItem = button
        }
    }

    override func shareItems(for url: URL) -> [Any] {
        var items: [Any] = []
        if case let .media(mediaItem) = context.viewState.currentItem, let caption = mediaItem.caption {
...
private struct HeaderView: View {
    ...
    var body: some View {
        ...
        case .media(let mediaItem):
            VStack(spacing: 0) {
                Text(mediaItem.sender.primaryInfo)
