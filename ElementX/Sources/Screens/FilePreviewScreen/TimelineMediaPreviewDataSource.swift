...
enum TimelineMediaPreviewItem: Equatable {
    ...
    @Observable class Media: NSObject, QLPreviewItem, Identifiable {
        ...
        // PG_CHANGED
        /// The MIME type of the media source, if available.
        var mimeType: String? {
            mediaSource?.mimeType
        }
        
        // PG_CHANGED
        /// The lowercased file extension including the leading dot (e.g. `".exe"`), or `nil` if none.
        var fileExtension: String? {
            FileTypeBlocklist.fileExtension(from: filename)
        }
        
        // PG_CHANGED
        /// Whether this file type is blocked from sharing and downloading.
        var isBlockedFileType: Bool {
            FileTypeBlocklist.isBlocked(mimeType: mimeType, filename: filename)
        }
    }
