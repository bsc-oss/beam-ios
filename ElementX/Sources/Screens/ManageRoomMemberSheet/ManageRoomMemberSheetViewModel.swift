...
class ManageRoomMemberSheetViewModel: ManageRoomMemberSheetViewModelType, ManageRoomMemberSheetViewModelProtocol {
    ...
    private func displayAlert(_ alertType: ManageRoomMemberSheetViewAlertType) {
        ...
        // PG_CHANGED
        let memberEmail = state.memberDetails.email
        ...
        case .kick:
            state.bindings.alertInfo = .init(id: alertType,
                                             ...
                                             // PG_CHANGED
                                             secondaryButton: .init(title: L10n.screenBottomSheetManageRoomMemberKickMemberConfirmationAction) { [weak self] in Task { await self?.kickMember(id: memberID, name: memberName, email: memberEmail, reason: reason) } },
        ...
        case .ban:
            state.bindings.alertInfo = .init(id: alertType,
                                             ...
                                             primaryButton: .init(title: L10n.actionCancel, role: .cancel) { },
                                             // PG_CHANGED
                                             secondaryButton: .init(title: L10n.screenBottomSheetManageRoomMemberBanMemberConfirmationAction) { [weak self] in Task { await self?.banMember(id: memberID, name: memberName, email: memberEmail, reason: reason) } },
                                             textFields: [.init(placeholder: L10n.commonReason,
                                                                text: binding,
                                                                autoCapitalization: .sentences,
                                                                autoCorrectionDisabled: false)])
        case .unban:
            state.bindings.alertInfo = .init(id: alertType,
                                             ...
                                             primaryButton: .init(title: L10n.actionCancel, role: .cancel) { },
                                             // PG_CHANGED
                                             secondaryButton: .init(title: L10n.screenBottomSheetManageRoomMemberUnbanMemberConfirmationAction) { [weak self] in Task { await self?.unbanMember(id: memberID, name: memberName, email: memberEmail) } })
        }
    }
    
    // PG_CHANGED
    private func kickMember(id: String, name: String?, email: String?, reason: String?) async {
        // PG_CHANGED
        let user = [name, email].compactMap { $0 }.first { !$0.isEmpty } ?? id
        let indicatorTitle = L10n.screenBottomSheetManageRoomMemberRemovingUser(user)
    ...
    private func banMember(id: String, name: String?, email: String?, reason: String?) async {
        // PG_CHANGED
        let user = [name, email].compactMap { $0 }.first { !$0.isEmpty } ?? id
        let indicatorTitle = L10n.screenBottomSheetManageRoomMemberBanningUser(user)
        showManageMemberIndicator(title: indicatorTitle)
    ...
    // PG_CHANGED
    private func unbanMember(id: String, name: String?, email: String?) async {
        // PG_CHANGED
        let user = [name, email].compactMap { $0 }.first { !$0.isEmpty } ?? id
        let indicatorTitle = L10n.screenBottomSheetManageRoomMemberUnbanningUser(user)
        showManageMemberIndicator(title: indicatorTitle)
