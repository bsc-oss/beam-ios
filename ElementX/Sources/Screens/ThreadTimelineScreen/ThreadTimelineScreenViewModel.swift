...
class ThreadTimelineScreenViewModel: ThreadTimelineScreenViewModelType, ThreadTimelineScreenViewModelProtocol {
    ...
    init(roomProxy: JoinedRoomProxyProtocol,
        ...
        let roomTitle = roomProxy.infoPublisher.value.displayName ?? roomProxy.id
        let roomSubtitle = "\(roomTitle)\(roomProxy.infoPublisher.value.dmFirstHeroEmailOrId.map { " (\($0))" } ?? "")"
        
        super.init(initialViewState: ThreadTimelineScreenViewState(roomSubtitle: roomSubtitle,
    ...
    private func updateRoomInfo(_ roomInfo: RoomInfoProxyProtocol) {
        let roomTitle = roomInfo.displayName ?? roomProxy.id
        // PG_CHANGED - displays email in DM room header view subtitle
        state.roomSubtitle = "\(roomTitle)\(roomInfo.dmFirstHeroEmailOrId.map { " (\($0))" } ?? "")"
