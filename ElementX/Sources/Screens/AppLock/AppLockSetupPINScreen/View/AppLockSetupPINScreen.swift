...
struct AppLockSetupPINScreen: View {
    ...
    var interactiveDismissDisabled: Bool {
        // PG_CHANGED
        context.viewState.isLoadedFromOnboarding || context.viewState.isLoggingOut
    }
    ...
    var toolbar: some ToolbarContent {
        // PG_CHANGED
        if !context.viewState.isLoadedFromOnboarding {
...
struct AppLockSetupPINScreen_Previews: PreviewProvider, TestablePreview {
    ...
    static let noLockService = AppLockServiceMock.mock(deviceHasLockScreen: false)
    
    static let createViewModel = AppLockSetupPINScreenViewModel(initialMode: .create,
                                                                isLoadedFromOnboarding: false,
                                                                appLockService: service)
    static let createOnboardingNoSecurityViewModel = AppLockSetupPINScreenViewModel(initialMode: .create,
                                                                                    isLoadedFromOnboarding: true,
                                                                                    appLockService: noLockService)
    ...
    static var previews: some View {
        ...
        NavigationStack {
            AppLockSetupPINScreen(context: createOnboardingNoSecurityViewModel.context)
        }
        .previewDisplayName("Create (Onboarding, No Device Security)")
