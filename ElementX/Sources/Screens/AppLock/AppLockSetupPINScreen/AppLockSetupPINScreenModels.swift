...
struct AppLockSetupPINScreenViewState: BindableState {
    ...
    /// PG_CHANGED: Whether the device has OS-level security (passcode, Face ID, or Touch ID).
    let deviceHasLockScreen: Bool
    ...
    var subtitle: String {
        guard mode == .unlock else {
            // PG_CHANGED: Show explanatory text during onboarding when device has no lock
            if isLoadedFromOnboarding, !deviceHasLockScreen {
                return L10n.pgScreenAppLockSetupPinContextOnboarding(InfoPlistReader.main.bundleDisplayName)
            }
