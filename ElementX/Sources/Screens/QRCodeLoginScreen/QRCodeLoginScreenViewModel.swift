...
class QRCodeLoginScreenViewModel: QRCodeLoginScreenViewModelType, QRCodeLoginScreenViewModelProtocol {
    ...
    // PG_CHANGED
    private var webAuthPresenter: PgQRCodeLoginWebAuthPresenter?
    ...
    override func process(viewAction: QRCodeLoginScreenViewAction) {
        ...
        case .dismiss, .errorAction(.dismiss):
            // PG_CHANGED: If a web auth session is in progress, cancel it too.
            webAuthPresenter?.cancel()
    ...
    private func handleScan(qrData: Data, loginService: QRCodeLoginServiceProtocol) {
        ...
        currentTask = progressPublisher
            ...
            .sink { [weak self] completion in
                ...
                // PG_CHANGED: add reciprocate mode
                case .confirmingLogin(let verificationUri, let verificationUriComplete):
                    // Reciprocation flow: open the verification URL using a Web Authentication Session
                    let urlString = verificationUriComplete ?? verificationUri
                    guard let url = URL(string: urlString) else {
                        MXLog.error("Invalid verification URL: \(urlString)")
                        return
                    }
                    
                    let presenter = PgQRCodeLoginWebAuthPresenter(presentationAnchor: appMediator.windowManager.mainWindow)
                    presenter.start(verificationURL: url) { [weak self] result in
                        guard let self else { return }
                        defer { self.webAuthPresenter = nil }
                        
                        switch result {
                        case .cancelledByUser:
                            // User cancelled the Web Authentication Session, reflect it in UI
                            state.state = .error(.webAuthCancelled)
                        case .completed:
                            // Whether completed or other error, resume the flow and let the SDK decide next steps
                            Task { await loginService.resumeReciprocation() }
                        }
                    }
