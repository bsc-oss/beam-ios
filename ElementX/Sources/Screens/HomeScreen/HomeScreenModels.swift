...
enum HomeScreenViewAction {
    ...
    // PG_CHANGED - dismiss the remote service-message banner.
    case dismissServiceMessage(id: Int)
...
struct HomeScreenViewState: BindableState {
    ...
    // PG_CHANGED - remote service-message (notice banner).
    var serviceMessage: PgServiceMessageDisplay?
    ...
    private var isFilteringRooms: Bool {
        bindings.filtersState.isFiltering || selectedSpaceFilter != nil
    }

    // PG_CHANGED: don't show the server-notice banner while searching or filtering the room list.
    var shouldShowServiceMessage: Bool {
        serviceMessage != nil && !bindings.isSearchFieldFocused && bindings.searchQuery.isEmpty && !isFilteringRooms
    }
    
    var shouldShowBanner: Bool {
        securityBannerMode.isShown || shouldShowNewSoundBanner || shouldShowServiceMessage // PG_CHANGED
    }
}
