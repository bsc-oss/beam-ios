...
//
//
// Modified by Belgian Secure Communications for Beam application on 2026-05-21
...
struct HomeScreenContent: View {
    ...
    private var roomList: some View {
        GeometryReader { geometry in
            ...
            .overlay {
                if context.viewState.shouldShowEmptyFilterState {
                    // PG_CHANGED: don't render in front of keystoragewarning when service message is shown
                    VStack(spacing: 0) {
                        topSection
    ...
    private var topSection: some View {
        ...
        if context.viewState.shouldShowFilters || context.viewState.shouldShowBanner {
            VStack(spacing: 0) {
                ...
                // PG_CHANGED - service-message banner shown above the security banner.
                if context.viewState.shouldShowServiceMessage, let serviceMessage = context.viewState.serviceMessage {
                    PgServiceMessageBanner(message: serviceMessage,
                                           onDismiss: serviceMessage.allowDismiss
                                               ? { context.send(viewAction: .dismissServiceMessage(id: serviceMessage.id)) }
                                               : nil)
                        .padding(.top, 8)
                        .padding(.bottom, context.viewState.securityBannerMode.isShown || context.viewState.shouldShowNewSoundBanner ? 8 : 0)
                }
