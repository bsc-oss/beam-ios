...
class HomeScreenViewModel: HomeScreenViewModelType, HomeScreenViewModelProtocol {
    ...
    // PG_CHANGED - service-message (notice banner) service.
    ...
    init(userSession: UserSessionProtocol,
        ...
        super.init(initialViewState: .init(userID: userSession.clientProxy.userID,
                                           serviceMessage: pgServiceMessageService.currentMessagePublisher.value,
                                           bindings: .init(filtersState: .init(appSettings: appSettings))),
                   mediaProvider: userSession.mediaProvider)
        
        // PG_CHANGED - subscribe to remote service-message updates.
        ...
        // PG_CHANGED - eagerly load the full user profile so the avatar shows the correct
        // state (picture / display name initial) immediately, instead of waiting for the room list
        Task {
            await self.userSession.clientProxy.loadUserProfileProxy()
        }
    ...
    override func process(viewAction: HomeScreenViewAction) {
        ...
        // PG_CHANGED
        case .dismissServiceMessage(let id):
            pgServiceMessageService.dismiss(messageID: id)
    ...
    private func updateRooms() {
        ...
        let spacesEnabled = appSettings.spacesEnabled
        
        for summary in roomSummaryProvider.roomListPublisher.value {
            // PG_CHANGED - filter out spaces from room list when spaces are disabled
            if !spacesEnabled, summary.isSpace {
                continue
            }
    ...
    private func finishAcceptInvite(roomProxy: InvitedRoomProxyProtocol) async {
        ...
        if roomProxy.info.isSpace {
            guard appSettings.spacesEnabled else {
                MXLog.info("Spaces are disabled, ignoring space invite acceptance")
                return
            }
    ...
    private func showDeclineInviteConfirmationAlert(roomID: String) async {
        ...
        // PG_CHANGED
        let roomPlaceholder = room.isDirect ? (room.inviter?.primaryInfo ?? room.name) : room.name
