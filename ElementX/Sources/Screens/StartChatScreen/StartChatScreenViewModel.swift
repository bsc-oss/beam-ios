...
class StartChatScreenViewModel: StartChatScreenViewModelType, StartChatScreenViewModelProtocol {
    ...
    private func setupBindings() {
        ...
        appSettings.$inviteFriendsEnabled
            .weakAssign(to: \.state.isInviteFriendsEnabled, on: self)
            .store(in: &cancellables)

        // PG_CHANGED
        appSettings.$joinRoomByAddressEnabled
            .weakAssign(to: \.state.isJoinRoomByAddressEnabled, on: self)
            .store(in: &cancellables)

        // PG_CHANGED - gates the room directory entry behind the RoomDirectorySearch flag
        appSettings.$roomDirectorySearchEnabled
    ...
    private func fetchUsers() {
        // PG_CHANGED - adapts searchQuery length requirement
