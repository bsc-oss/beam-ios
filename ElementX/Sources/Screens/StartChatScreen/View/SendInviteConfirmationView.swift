...
struct SendInviteConfirmationView: View {
    ...
    private var subtitle: String {
        let string: String
        // PG_CHANGED - show primaryInfo (+ secondaryInfo when present), email-aware via PgUserDescribing
        if let secondaryInfo = userToInvite.user.secondaryInfo {
            string = L10n.commonNameAndId(userToInvite.user.primaryInfo, secondaryInfo)
        } else {
            string = userToInvite.user.primaryInfo
        }
    ...
    private var header: some View {
        VStack(spacing: 16) {
            LoadableAvatarImage(url: userToInvite.avatarURL,
                                ...
                                // PG_CHANGED - pass email for the placeholder avatar
                                email: userToInvite.user.email,
