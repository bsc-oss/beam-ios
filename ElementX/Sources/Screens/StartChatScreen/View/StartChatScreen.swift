...
struct StartChatScreen: View {
    ...
    private var mainContent: some View {
        ...
        // PG_CHANGED
        if context.viewState.isInviteFriendsEnabled {
            inviteFriendsSection
        }
        // PG_CHANGED
        if context.viewState.isJoinRoomByAddressEnabled {
...
struct StartChatScreen_Previews: PreviewProvider, TestablePreview {
    ...
    static func makeViewModel(searchQuery: String = "", searchProfilesResult: Result<[UserProfileProxy], UserDiscoveryErrorType> = .success([.mockAlice])) -> StartChatScreenViewModel {
        let appSettings = AppSettings()
        appSettings.publicSearchEnabled = true
        let userSession = UserSessionMock(.init(clientProxy: ClientProxyMock(.init(userID: "@userid:example.com"))))
        let userDiscoveryService = UserDiscoveryServiceMock()
        userDiscoveryService.searchProfilesWithReturnValue = searchProfilesResult
        let viewModel = StartChatScreenViewModel(userSession: userSession,
                                                 analytics: ServiceLocator.shared.analytics,
                                                 userIndicatorController: UserIndicatorControllerMock(),
                                                 userDiscoveryService: userDiscoveryService,
                                                 appSettings: appSettings)
        viewModel.state.bindings.searchQuery = searchQuery
        return viewModel
    }
}
