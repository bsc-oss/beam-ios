...
class RoomScreenViewModel: RoomScreenViewModelType, RoomScreenViewModelProtocol {
    ...
    init(userSession: UserSessionProtocol,
        ...
        let viewState = RoomScreenViewState(roomTitle: roomProxy.infoPublisher.value.displayName ?? roomProxy.id,
                                            ...
                                            roomSubtitle: roomProxy.infoPublisher.value.dmFirstHeroEmailOrId,
                                            ...
                                            // PG_CHANGED - hide the history sharing icon for DMs (history sharing doesn't apply to a DM)
                                            roomHistorySharingState: roomProxy.infoPublisher.value.isDirect ? nil : roomProxy.infoPublisher.value.historySharingState)
    ...
    private func updateVerificationBadge() async {
        ...
        guard let userIdentity else {
            // PG_CHANGED - unknown user identity is a valid use case i.e. when chatting with an unverified user (room created by unknown mxid).
            MXLog.warning("User identity is unknown at this point")
            state.dmRecipientVerificationState = .notVerified
            return
        }
    ...
    private func updateRoomInfo(_ roomInfo: RoomInfoProxyProtocol) {
        ...
        // PG_CHANGED - displays email in DM room header view subtitle
        state.roomSubtitle = roomInfo.dmFirstHeroEmailOrId
        ...
        // PG_CHANGED - hide call button when user is alone in room
        state.isUserAloneInRoom = !roomInfo.hasOtherJoinedMember
        ...
        // PG_CHANGED - hide the history sharing icon for DMs (history sharing doesn't apply to a DM)
        state.roomHistorySharingState = roomInfo.isDirect ? nil : roomInfo.historySharingState
    }
