...
final class ComposerToolbarViewModel: ComposerToolbarViewModelType, ComposerToolbarViewModelProtocol {
    ...
    private func parseUserMentionsMarkdown(_ text: String, callback: (NSRange, URL) -> Void) {
        ...
        let pattern = "\\[(.*?)\\]\\(https://share\\.beam\\.belgium\\.be/#/(@.*?)\\)"
        ...
        for match in matches.sorted(by: { $0.range.length > $1.range.length }) {
            ...
            // PG_CHANGED
            let fullURLString = "https://share.beam.belgium.be/#/\(userID)"
...
extension ComposerToolbarViewModel {
    ...
    static func mock(focused: Bool = false,
        ...
        let suggestions: [SuggestionItem] = if hasSuggestions {
            // PG_CHANGED - SuggestionItem.User requires an email
            [.init(suggestionType: .user(.init(id: "@user_mention_1:matrix.org", displayName: "User 1", avatarURL: nil, email: "user_mention_1@matrix.org")), range: .init(), rawSuggestionText: ""),
             .init(suggestionType: .user(.init(id: "@user_mention_2:matrix.org", displayName: "User 2", avatarURL: .mockMXCUserAvatar, email: "user_mention_2@matrix.org")), range: .init(), rawSuggestionText: "")]
        } else {
            []
        }
