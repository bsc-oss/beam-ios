...
final class CompletionSuggestionService: CompletionSuggestionServiceProtocol {
        ...
        var membersSuggestion = members
            .compactMap { member -> SuggestionItem? in
                guard member.userID != ownUserID,
                      ...
                      // PG_CHANGED
                      Self.shouldIncludeMember(userID: member.userID, displayName: member.displayName, email: member.email, searchText: suggestionTrigger.text) else {
                    return nil
                }
                // PG_CHANGED
                return .init(suggestionType: .user(.init(id: member.userID, displayName: member.displayName, avatarURL: member.avatarURL, email: member.email)), range: suggestionTrigger.range, rawSuggestionText: suggestionTrigger.text)
            }
        
        if canMentionAllUsers,
           !roomProxy.infoPublisher.value.isDM,
           // PG_CHANGED - shouldIncludeMember takes an email parameter
           Self.shouldIncludeMember(userID: PillUtilities.atRoom, displayName: PillUtilities.everyone, email: nil, searchText: suggestionTrigger.text) {
            membersSuggestion
    ...
    // PG_CHANGED
    private static func shouldIncludeMember(userID: String, displayName: String?, email: String?, searchText: String) -> Bool {
        ...
        // PG_CHANGED
        let containedInEmail: Bool
        if let email {
            containedInEmail = email.localizedStandardContains(searchText)
        } else {
            containedInEmail = false
        }
        
        // PG_CHANGED
        return containedInUserID || containedInDisplayName || containedInEmail
    }
