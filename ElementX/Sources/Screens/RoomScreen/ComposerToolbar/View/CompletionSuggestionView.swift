...
struct CompletionSuggestionView: View {
    ...
    var body: some View {
        if items.isEmpty {
            ...
            ZStack {
                // PG_CHANGED
                MentionSuggestionItemView(mediaProvider: nil, item: .init(suggestionType: .user(.init(id: "", displayName: nil, avatarURL: nil, email: nil)), range: .init(), rawSuggestionText: ""))
...
struct CompletionSuggestion_Previews: PreviewProvider, TestablePreview {
    static let multipleItems: [SuggestionItem] = (0...10).map { index in
        // PG_CHANGED
        .init(suggestionType: .user(.init(id: "\(index)", displayName: "\(index)", avatarURL: nil, email: "\(index)@matrix.org")), range: .init(), rawSuggestionText: "")
    }
    ...
    static var previews: some View {
        ...
        VStack(spacing: 8) {
            CompletionSuggestionView(mediaProvider: MediaProviderMock(configuration: .init()),
                                     // PG_CHANGED
                                     items: [.init(suggestionType: .user(.init(id: "@user_mention_1:matrix.org", displayName: "User 1", avatarURL: nil, email: "user_mention_1@matrix.org")), range: .init(), rawSuggestionText: ""),
                                             .init(suggestionType: .user(.init(id: "@user_mention_2:matrix.org", displayName: "User 2", avatarURL: .mockMXCUserAvatar, email: "user_mention_2@matrix.org")), range: .init(), rawSuggestionText: "")]) { _ in }
        }
