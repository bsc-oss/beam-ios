...
struct AuthenticationStartScreen: View {
        ...
        GeometryReader { geometry in
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if let serviceMessage = context.viewState.serviceMessage {
                        PgServiceMessageBanner(message: serviceMessage, onDismiss: nil)
        ...
        VStack(spacing: 0) {
            ...
            if verticalSizeClass == .regular {
                ...
                AuthenticationStartLogo()
                    .padding(EdgeInsets(top: 0, leading: 70, bottom: 60, trailing: 70))
            }
            ...
            VStack(spacing: 8) {
                ...
                // Title
                VStack(spacing: 0) {
                    Text(L10n.pgScreenOnboardingWelcomeTitle)
                        .foregroundColor(.compound.textPrimary)
                        .multilineTextAlignment(.center)

                    // PG_CHANGED - custom second title line (replaces upstream's welcome message); centered per upstream
                    Text(L10n.pgScreenOnboardingWelcomeTitle2)
                        .foregroundColor(.compound.textTertiary)
                        .multilineTextAlignment(.center)
                }
                .font(.compound.headingLGBold)
                
                // Welcome message
                Text(L10n.pgScreenOnboardingWelcomeMessage)
        ...
        VStack(spacing: 16) {
            if context.viewState.showQRCodeLoginButton {
                Button { context.send(viewAction: .loginWithQR) } label: {
                    // PG_CHANGED
                    Label(L10n.pgScreenOnboardingSignInWithQrCode, icon: \.qrCode)
                }
            ...
            Button { context.send(viewAction: .login) } label: {
                // PG_CHANGED
                Text(L10n.pgScreenOnboardingSignInManually)
            }
            .buttonStyle(.compound(.primary))
    ...
    var developerOptionsButton: some View {
        if context.viewState.showDeveloperOptions, !ProcessInfo.isRunningTests { // PG_CHANGED - gate by developerOptionsEnabled (like SettingsScreen) instead of build configuration
