...
struct UserProfileScreen: View {
    ...
    private var userIDSection: some View {
        if let userProfile = context.viewState.userProfile {
            ...
            CopyTextButton(content: userProfile.userID, font: .compound.bodyMD, iconSize: .xSmall)
        }
    }
    ...
    private var footer: some View {
        VStack(spacing: 8) {
            // PG_CHANGED - adds custom profile fields
            let email = context.viewState.userProfile?.email
            let function = context.viewState.userProfile?.function
            let department = context.viewState.userProfile?.department
            
            PgCustomProfileFieldsView(email: email, function: function, department: department)
                .padding(.top, 24)
            ...
            HStack(spacing: 8) {
                ...
                if let roomID = context.viewState.dmRoomID {
                    // PG_CHANGED - adopt upstream's voice/video call distinction
                    Button {
                ...
                // PG_CHANGED - gate profile sharing behind the feature flag
                if context.viewState.isShareProfileEnabled, let permalink = context.viewState.permalink {
...
struct UserProfileScreen_Previews: PreviewProvider, TestablePreview {
    ...
    static func makeViewModel(userID: String) -> UserProfileScreenViewModel {
        ...
        // PG_CHANGED - adds own profile fields
        clientProxyMock.profileForReturnValue = .success(.mockAlice)
