...
class AuthenticationFlowCoordinator: FlowCoordinatorProtocol {
    ...
    // PG_CHANGED - stored to clear pending OIDC state when flow is cancelled
    ...
    private func configureStateMachine() {
        ...
        // PG_CHANGED
        stateMachine.addRoutes(event: .continueWithOIDC, transitions: [.emailInputScreen => .oidcAuthentication,
        ...
        // PG_CHANGED
        stateMachine.addRoutes(event: .cancelledOIDCAuthentication(previousState: .emailInputScreen), transitions: [.oidcAuthentication => .emailInputScreen]) { [weak self] _ in
            self?.pgEmailInputScreenCoordinator?.clearPendingOIDC()
        }
    ...
    private func showEmailInputScreen(authenticationFlow: AuthenticationFlow) {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            switch action {
            case .continueWithOIDC(let oidcData, let window):
                // PG_CHANGED - proceeds only if we are still in the email screen and there isn't an OIDC presenter ongoing.
                // Prevents double triggering if the user taps multiple times quickly.
                guard self.stateMachine.state == .emailInputScreen, self.oidcPresenter == nil else { return }
                stateMachine.tryEvent(.continueWithOIDC, userInfo: (oidcData, window))
            }
        }
        .store(in: &cancellables)
