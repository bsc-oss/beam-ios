...
class AuthenticationFlowCoordinator: FlowCoordinatorProtocol {
    ...
    enum State: StateType {
        ...
        /// The email input screen.
        // PG_CHANGED
        case emailInputScreen
        // PG_CHANGED - removed `oidcAuthentication` state: OAuth is now presented outside the state machine (upstream refactor)
    ...
    enum Event: EventType {
        ...
        // PG_CHANGED
        case inputEmail
        // PG_CHANGED - dropped duplicate `reportProblem` (now provided by upstream below); removed OIDC events (OAuth no longer in state machine)
    ...
    // PG_CHANGED - stored to clear pending OAuth state when flow is cancelled
    private var pgEmailInputScreenCoordinator: PgEmailInputScreenCoordinator?
    ...
    init(authenticationService: AuthenticationServiceProtocol,
         ...
         pgServiceMessageService: PgServiceMessageServiceProtocol,
        ...
        self.pgServiceMessageService = pgServiceMessageService
    ...
    private func configureStateMachine() {
        ...
        // PG_CHANGED
        stateMachine.addRoutes(event: .confirmServer(.login), transitions: [.startScreen => .emailInputScreen]) { [weak self] _ in
            self?.showEmailInputScreen(authenticationFlow: .login)
        }
        ...
        // PG_CHANGED
        stateMachine.addRoutes(event: .cancelledServerConfirmation, transitions: [.emailInputScreen => .startScreen])
        ...
        stateMachine.addRoutes(event: .signedIn, transitions: [.qrCodeLoginScreen => .complete,
                                                               ...
                                                               // PG_CHANGED - OAuth authentication is launched from the email input screen
                                                               .emailInputScreen => .complete,
    ...
    // PG_CHANGED
    private func showEmailInputScreen(authenticationFlow: AuthenticationFlow) {
        // Reset the service back to the default homeserver before continuing. This ensures
        // we check that registration is supported if it was previously configured for login.
        authenticationService.reset()
        
        let parameters = PgEmailInputScreenCoordinatorParameters(authenticationService: authenticationService,
                                                                 pgEmailValidationService: pgEmailValidationService,
                                                                 userIndicatorController: userIndicatorController,
                                                                 appSettings: appSettings)
        let coordinator = PgEmailInputScreenCoordinator(parameters: parameters)
        pgEmailInputScreenCoordinator = coordinator
        
        coordinator.actions.sink { [weak self] action in
            guard let self else { return }
            
            switch action {
            // PG_CHANGED - email screen produced OAuth data; present OAuth directly (upstream moved OAuth out of the state machine)
            case .continueWithOAuth(let oAuthData, let window):
                // Proceed only if we are still on the email screen and there isn't an OAuth presenter ongoing.
                // Prevents double triggering if the user taps multiple times quickly.
                guard self.stateMachine.state == .emailInputScreen, self.oAuthPresenter == nil else { return }
                showOAuthAuthentication(oAuthData: oAuthData, presentationAnchor: window)
            }
        }
        .store(in: &cancellables)
    ...
    private func showOAuthAuthentication(oAuthData: OAuthAuthorizationDataProxy, presentationAnchor: UIWindow) {
        ...
        Task {
            ...
            case .failure:
                // PG_CHANGED - re-enable the email input screen's continue button when OAuth is cancelled/fails (no-op outside the email flow)
                pgEmailInputScreenCoordinator?.clearPendingOAuth()
            }
            oAuthPresenter = nil
        }
    }
