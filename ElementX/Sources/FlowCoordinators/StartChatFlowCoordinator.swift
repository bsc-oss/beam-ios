...
class StartChatFlowCoordinator: FlowCoordinatorProtocol {
    ...
    // PG_CHANGED - sets spaceSelectionMode to .none if spacesEnabled is false
    private func spaceSelectionMode(preSelectedSpace: SpaceServiceRoom? = nil) -> CreateRoomScreenSpaceSelectionMode {
        flowParameters.appSettings.spacesEnabled ? .editableSpacesList(preSelectedSpace: preSelectedSpace) : .none
    }
    ...
    private func configureStateMachine() {
        ...
        stateMachine.addRoutes(event: .createRoom(isSpace: false), transitions: [.initial => .createRoom]) { [weak self] context in
            ...
            // PG_CHANGED - sets spaceSelectionMode to .none if spacesEnabled is false
            self?.presentCreateRoomScreen(isSpace: false, spaceSelectionMode: self?.spaceSelectionMode(preSelectedSpace: space) ?? .none, isRoot: true)
        }
