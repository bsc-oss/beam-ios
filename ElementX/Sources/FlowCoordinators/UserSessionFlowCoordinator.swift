...
class UserSessionFlowCoordinator: FlowCoordinatorProtocol {
    ...
    private func setupObservers() {
        ...
        // Note: changing spacesEnabled requires an app restart to take effect for the tab bar.
        userSession.clientProxy.spaceService.topLevelSpacesPublisher
            .combineLatest(flowParameters.appSettings.$createSpaceEnabled, flowParameters.appSettings.$spacesEnabled)
            .map { topLevelSpaces, isCreateSpaceEnabled, isSpacesEnabled in
                !isSpacesEnabled || (!isCreateSpaceEnabled && topLevelSpaces.isEmpty) ? .hidden : nil
            }
