...
class SettingsFlowCoordinator: FlowCoordinatorProtocol {
    ...
    private func presentUserDetailsEditScreen() {
        let coordinator = UserDetailsEditScreenCoordinator(parameters: .init(orientationManager: flowParameters.windowManager,
                                                                             ...
                                                                             isScreenLoadedFromOnboardingFlow: false))
        coordinator.actions
            .sink { [weak self] action in
                switch action {
                case .dismiss:
    ...
    // PG_CHANGED
    private func pgPresentQRCodeLoginScreen() {
        // Use the existing session to run the reciprocation flow for linking a device.
        let linkService = PgQRCodeReciprocateService(userSession: flowParameters.userSession)
        let coordinator = QRCodeLoginScreenCoordinator(parameters: .init(mode: .login(linkService),
                                                                         // From Settings (linking), manual sign-in isn't applicable.
                                                                         canSignInManually: false,
                                                                         orientationManager: flowParameters.windowManager,
                                                                         appMediator: flowParameters.appMediator))
        coordinator.start()
        coordinator.actionsPublisher.sink { [weak self] (action: QRCodeLoginScreenCoordinatorAction) in
            guard let self else { return }
            switch action {
            case .cancel, .startOver:
                navigationStackCoordinator.setSheetCoordinator(nil)
            case .signInManually:
                navigationStackCoordinator.setSheetCoordinator(nil)
            // From settings, manual sign-in isn’t applicable; just dismiss.
            case .signedIn:
                navigationStackCoordinator.setSheetCoordinator(nil)
            // Already signed in; nothing to do in settings flow.
            case .requestOIDCAuthorisation(let url, let continuation):
                presentAccountManagementURL(url, continuation: continuation)
            case .linkedDevice:
                navigationStackCoordinator.setSheetCoordinator(nil)
        ...
        .store(in: &cancellables)
        let modalStackCoordinator = NavigationStackCoordinator()
        modalStackCoordinator.setRootCoordinator(coordinator)
        navigationStackCoordinator.setSheetCoordinator(modalStackCoordinator)
    }
