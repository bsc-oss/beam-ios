...
class OnboardingFlowCoordinator: FlowCoordinatorProtocol {
    ...
    var shouldStart: Bool {
        ...
        // PG_CHANGED - adds custom profile fields
        return isNewLogin || requiresVerification || requiresAppLockSetup || requiresAnalyticsSetup || requiresNotificationsSetup || requiresProfileCreation
    }
    ...
    private var requiresAppLockSetup: Bool {
        // PG_CHANGED: App lock is only mandatory if the device doesn't have OS-level security
        let isActuallyMandatory = appSettings.appLockIsMandatory && !appLockService.deviceHasLockScreen
        return isActuallyMandatory && !appLockService.isEnabled
    }
    ...
    // PG_CHANGED - adds custom profile fields
    private var requiresProfileCreation: Bool {
        guard !appSettings.hasRunProfileCreationOnboarding else { return false }

        // If userProfile is not set, skip profile creation screen.
        let userProfile = userSession.clientProxy.userProfileProxyPublisher.value
        guard let userProfile else { return false }

        return userProfile.isProfileInitialized != true
    }
    ...
    private func configureStateMachine() {
        ...
        stateMachine.addRouteMapping { [weak self] event, fromState, _ in
            ...
            switch (fromState, requiresVerification, requiresAppLockSetup, requiresAnalyticsSetup, requiresNotificationsSetup, requiresProfileCreation) {
            ...
            case (.identityConfirmation, _, _, _, _, _):
                if event == .nextSkippingIdentityConfirmed {
                    ...
                    switch (requiresAppLockSetup, requiresAnalyticsSetup, requiresNotificationsSetup, requiresProfileCreation) {
                    case (true, _, _, _):
                        return .appLockSetup
                    case (false, true, _, _):
                        return .analyticsPrompt
            ...
            case (.identityConfirmed, _, true, _, _, _):
                return .appLockSetup
            case (.identityConfirmed, _, false, true, _, _):
                return .analyticsPrompt
            case (.identityConfirmed, _, false, false, true, _):
                return .notificationPermissions
            case (.identityConfirmed, _, false, false, false, _):
                return .finished
                
            case (.appLockSetup, _, _, true, _, _):
                return .analyticsPrompt
            case (.appLockSetup, _, _, false, true, _):
                return .notificationPermissions
            case (.appLockSetup, _, _, false, false, _):
                return .finished
                
            case (.analyticsPrompt, _, _, _, true, _):
                return .notificationPermissions
            case (.analyticsPrompt, _, _, _, false, _):
                return .finished
                
            case (.notificationPermissions, _, _, _, _, true):
                return .profileCreation
            case (.notificationPermissions, _, _, _, _, false):
                return .finished
                
            // PG_CHANGED - adds custom profile fields
            case (.profileCreation, _, _, _, _, true):
                return .profileCreation
            case (.profileCreation, _, _, _, _, false):
                return .finished
        ...
        stateMachine.addAnyHandler(.any => .any) { [weak self] context in
            ...
            // PG_CHANGED - adds custom profile fields
            case (_, _, .profileCreation):
                presentProfileCreationScreen()
            case (_, _, .finished):
                isNewLogin = false
    ...
    // PG_CHANGED - adds custom profile fields
    private func presentProfileCreationScreen() {
        let coordinator = UserDetailsEditScreenCoordinator(parameters: .init(orientationManager: windowManager,
