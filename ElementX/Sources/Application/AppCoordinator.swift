...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    // PG_CHANGED
    private let pgEmailValidationService: PgEmailValidationServiceProtocol
    // PG_CHANGED - service that fetches the remote service-message (notice banner).
    private let pgServiceMessageService: PgServiceMessageServiceProtocol
    ...
    init(appDelegate: AppDelegate) {
        ...
        // PG_CHANGED
        pgEmailValidationService = PgEmailValidationService(pgServiceUrl: appSettings.pgServiceUrl)
        
        // PG_CHANGED - service-message banner
        pgServiceMessageService = PgServiceMessageService(pgNoticeUrl: appSettings.pgNoticeUrl,
                                                          appSettings: appSettings)
    ...
    func start() {
        ...
        // Ensure the service-message banner is fetched on cold launch too. Relying only on
        // didBecomeActive can miss the initial auth flow if the observer is attached after launch.
        Task { await pgServiceMessageService.refresh() }
    ...
    func handleExternalLinkConfirmation(url: URL, openURLAction: @escaping (URL) -> Void) -> Bool {
        guard let externalLinkParameters = url.externalLinkParameters else {
            return false
        }
        navigationRootCoordinator.alertInfo = .init(id: .init(),
                                                    title: L10n.pgDialogOpenLinkTitle,
                                                    message: L10n.pgDialogOpenLinkMessage(externalLinkParameters.originalURL.absoluteString),
                                                    primaryButton: .init(title: L10n.actionCancel, role: .cancel, action: nil),
                                                    secondaryButton: .init(title: L10n.actionContinue) { openURLAction(externalLinkParameters.originalURL) })
        return true
    }
    ...
    private func performMigrationsIfNecessary(from oldVersion: Version, to newVersion: Version) {
        ...
        // PG_CHANGED — Upstream's version-gated migrations are intentionally REMOVED here.
        // They don't apply to our fork (our users only ever ran our builds) and their triggers
        // (`oldVersion < Version(1.6.x / 25.x)`) would be always-true for our SemVer versions,
        // so they'd re-run on every release. Deleted rather than kept dormant on purpose: a
        // future upstream merge that adds or changes a migration will CONFLICT at this spot,
        // forcing us to review whether it needs a fork-specific port (with a SemVer-appropriate
        // trigger, as PgBadgeResetMigration does). Do not re-add upstream migrations verbatim.

        // PG_CHANGED — Reset app icon badge to 0 for users upgrading to v1.0.3.
        // The sygnal push gateway was reconfigured to stop sending badge counts (the feature was buggy),
        // but existing users who already had a badge pinned to their app icon would never see it cleared.
        // New installs and users without a badge at the time of deployment are unaffected.
        if PgBadgeResetMigration.shouldReset(from: oldVersion) {
            MXLog.info("Migrating to version 1.0.3, resetting app icon badge count to 0.")
            PgBadgeResetMigration.reset()
        }
    ...
    private func performUserSessionMigrations(_ userSession: UserSessionProtocol) async {
        ...
        // PG_CHANGED — Upstream's version-gated user-session migrations are intentionally REMOVED
        // here, for the same reason as in performMigrationsIfNecessary above: they don't apply to
        // our fork and their CalVer triggers are always-true for our SemVer versions. Deleted (not
        // kept dormant) so a future upstream merge that touches these migrations CONFLICTS here and
        // forces a conscious review. The `performSettingsToAccountDataMigration` helper that the
        // removed 25.06.0 block used was deleted with it. Do not re-add upstream migrations verbatim.
    ...
    private func startAuthentication() {
        ...
        let coordinator = AuthenticationFlowCoordinator(authenticationService: authenticationService,
                                                        // PG_CHANGED
                                                        pgEmailValidationService: pgEmailValidationService,
                                                        pgServiceMessageService: pgServiceMessageService,
    ...
    private func startAuthenticationSoftLogout() {
        ...
        Task {
            let credentials = SoftLogoutScreenCredentials(userID: userSession.clientProxy.userID,
                                                          ...
                                                          userDisplayName: userSession.clientProxy.userProfileProxyPublisher.value?.displayName ?? "",
    ...
    private func observeAppLockChanges() {
        appLockFlowCoordinator.actions.sink { [weak self] action in
            ...
            case .forceLogout:
                // PG_CHANGED
                // Handle the case where the user is already logged out but app lock is still enabled
                // (e.g., when the user taps "Forgot PIN" after session restoration failed)
                if userSession == nil {
                    appLockFlowCoordinator.appLockService.disable()
                    windowManager.switchToMain()
                } else {
    ...
    private static func setupSentry(bugReportService: BugReportServiceProtocol, appSettings: AppSettings, analytics: AnalyticsService) {
        ...
        // PG_CHANGED - adjust Sentry environment value based on redefined appBuildType
        options.environment = AppSettings.appBuildType.scheme.description
    ...
    private func scheduleDelayedSyncStop() {
        guard backgroundTask == nil else {
            MXLog.info("PG_CHANGED - Background task already exists, skipping scheduleDelayedSyncStop")
            return
        }
        
        let remainingTime = UIApplication.shared.backgroundTimeRemaining
        MXLog.info("PG_CHANGED - Scheduling delayed sync stop. Remaining background time: \(remainingTime) seconds")
        
        backgroundTask = appMediator.beginBackgroundTask {
            let remainingTime = UIApplication.shared.backgroundTimeRemaining
            MXLog.info("PG_CHANGED - Background task is about to expire. Remaining time: \(remainingTime) seconds")
            ...
            MXLog.info("PG_CHANGED - Background task expiration: calling stopSync")
            self.stopSync(isBackgroundTask: true) {
                MXLog.info("PG_CHANGED - Background task expiration: stopSync completed, ending background task")
                self.endActiveBackgroundTask()
            }
            MXLog.info("PG_CHANGED - Background task expiration: handler returning to iOS")
        }
        
        MXLog.info("PG_CHANGED - Started background task: \(String(describing: backgroundTask))")
    }
    ...
    private func applicationDidBecomeActive() {
        ...
        startSync()
        // PG_CHANGED - refresh the service-message banner on each foreground (throttled internally to once per 15 minutes).
        Task { await pgServiceMessageService.refresh() }
    }
    
    private func endActiveBackgroundTask() {
        guard let backgroundTask else {
            MXLog.info("PG_CHANGED - endActiveBackgroundTask called but no active background task")
            return
        }
        
        // PG_CHANGED
        let remainingTime = UIApplication.shared.backgroundTimeRemaining
        MXLog.info("PG_CHANGED - Ending background task: \(backgroundTask). Remaining time: \(remainingTime) seconds")
        appMediator.endBackgroundTask(backgroundTask)
        self.backgroundTask = nil
        MXLog.info("PG_CHANGED - Background task ended successfully")
    }
