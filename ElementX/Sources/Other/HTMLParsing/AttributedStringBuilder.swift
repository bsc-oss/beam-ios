...
struct AttributedStringBuilder: AttributedStringBuilderProtocol {
    ...
    func fromPlain(_ string: String?) -> AttributedString? {
        ...
        // PG_CHANGED: Mark external links for confirmation
        markExternalLinksForConfirmation(mutableAttributedString)
    ...
    func fromHTML(_ htmlString: String?) -> AttributedString? {
        ...
        addLinksAndMentions(mutableAttributedString)
        addMatrixEntityPermalinkAttributesTo(mutableAttributedString)
        // PG_CHANGED: Mark external links for confirmation after all links have been added
        markExternalLinksForConfirmation(mutableAttributedString)
    ...
    // PG_CHANGED: Mark external links for confirmation
    private func markExternalLinksForConfirmation(_ attributedString: NSMutableAttributedString) {
        attributedString.enumerateAttribute(.link, in: .init(location: 0, length: attributedString.length), options: []) { value, range, _ in
            guard let url = value as? URL else {
                return
            }
            
            // Skip if already marked for phishing confirmation
            guard !url.requiresConfirmation else {
                return
            }
            
            // Skip Matrix permalinks (they navigate within the app)
            guard parseMatrixEntityFrom(uri: url.absoluteString) == nil else {
                return
            }
            
            handleExternalLink(for: attributedString, in: range, originalURL: url)
        }
    }
    
    private func handleExternalLink(for attributedString: NSMutableAttributedString,
                                    in range: NSRange,
                                    originalURL: URL) {
        attributedString.removeAttribute(.link, range: range)
        
        var urlComponents = URLComponents()
        urlComponents.scheme = URL.externalLinkConfirmationScheme
        urlComponents.host = ""
        let parameters = ExternalLinkParameters(originalURL: originalURL)
        urlComponents.queryItems = parameters.urlQueryItems
