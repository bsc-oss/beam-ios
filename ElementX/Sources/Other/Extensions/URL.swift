...
// MARK: - PG_CHANGED: External Link Confirmation URL

extension URL {
    static let externalLinkConfirmationScheme = "confirmexternal"
    
    var requiresExternalLinkConfirmation: Bool {
        scheme == Self.externalLinkConfirmationScheme
    }
    
    var externalLinkParameters: ExternalLinkParameters? {
        guard requiresExternalLinkConfirmation,
              let queryItems = URLComponents(url: self, resolvingAgainstBaseURL: true)?.queryItems else {
            return nil
        }
        return ExternalLinkParameters(queryItems: queryItems)
    }
}

struct ExternalLinkParameters {
    static let originalURLKey = "originalURL"
    
    let originalURL: URL
    
    var urlQueryItems: [URLQueryItem] {
        [URLQueryItem(name: Self.originalURLKey, value: originalURL.absoluteString)]
    }
}

extension ExternalLinkParameters {
    init?(queryItems: [URLQueryItem]) {
        guard let originalURLString = queryItems.first(where: { $0.name == Self.originalURLKey })?.value,
