...
extension String {
    /// Minimal client‑side verification (basic sanity) and deferring authoritative validation to the server.
    /// This is not RFC compliant.
    var isLikelyEmail: Bool {
        // Note: most checks could be packed in one regex but multi-step approach is used for clarity and precise control.

        // 0. Trim spaces/newlines the user may have typed
        ...
        guard let atIndex = trimmed.firstIndex(of: "@"),
              ...
              trimmed[atIndex...].dropFirst().contains("@") == false else {
            return false
        }
        ...
        // 7. structural pattern (case insensitive)
        guard Self.basicEmailStructureRegex.firstMatch(in: trimmed, options: [], range: NSRange(trimmed.startIndex..., in: trimmed)) != nil else {
            return false
        }
        return true
    }
    
    private static let basicEmailStructureRegex: NSRegularExpression = {
