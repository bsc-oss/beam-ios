...
struct AvatarHeaderView<Footer: View>: View {
    ...
    init(room: RoomDetails,
        ...
        // PG_CHANGED - hide the history sharing badges for DMs (history sharing doesn't apply to a DM)
        if let state = room.historySharingState, !room.isDirect {
    ...
    init(accountOwner: RoomMemberDetails,
        ...
        // PG_CHANGED
        title = dmRecipientProfile.primaryInfo
        subtitle = dmRecipientProfile.secondaryInfo
    ...
    init(user: UserProfileProxy,
        ...
        // PG_CHANGED
        title = user.primaryInfo
        // PG_CHANGED - adds custom profile fields
        subtitle = hideSubtitle ? nil : user.secondaryInfo
