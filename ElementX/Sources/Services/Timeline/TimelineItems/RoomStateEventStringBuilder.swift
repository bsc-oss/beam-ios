...
struct RoomStateEventStringBuilder {
    ...
    func buildString(for change: MembershipChange?,
        ...
        // PG_CHANGED
        let member = [memberDisplayName, memberEmail].compactMap { $0 }.first { !$0.isEmpty } ?? memberUserID
        // PG_CHANGED - fall back to the sender's email or ID instead of just the ID
        let senderDisplayName = sender.disambiguatedDisplayName ?? sender.emailOrId
