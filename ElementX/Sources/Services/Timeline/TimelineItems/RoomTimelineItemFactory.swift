...
struct RoomTimelineItemFactory: RoomTimelineItemFactoryProtocol {
    ...
    func buildTimelineItem(for eventItemProxy: EventTimelineItemProxy, isDM: Bool) -> RoomTimelineItemProtocol? {
        ...
        // PG_CHANGED
        case .roomMembership(userId: let userID, let displayName, userEmail: let email, change: let change, let reason):
            if isDM, change == .joined, userID == self.userID {
                return nil
            }
            // PG_CHANGED
            return buildStateMembershipChangeTimelineItem(for: eventItemProxy, memberUserID: userID, memberDisplayName: displayName, memberEmail: email, membershipChange: change, reason: reason, isOutgoing: isOutgoing)
    ...
    private func buildEmoteTimelineItem(for eventItemProxy: EventTimelineItemProxy,
        ...
        EmoteRoomTimelineItem(id: eventItemProxy.id,
                              ...
                              sender: eventItemProxy.sender,
                              // PG_CHANGED
                              content: buildEmoteTimelineItemContent(senderDisplayName: eventItemProxy.sender.displayName, senderEmail: eventItemProxy.sender.email, senderID: eventItemProxy.sender.id, messageContent: emoteMessageContent),
                              properties: .init(replyDetails: buildTimelineItemReplyDetails(messageLikeContent.inReplyTo),
                                                isThreaded: messageLikeContent.threadRoot != nil,
                                                threadSummary: buildTimelineItemThreadSummary(messageLikeContent.threadSummary),
                                                isEdited: messageContent.isEdited,
    ...
    // PG_CHANGED
    private func buildEmoteTimelineItemContent(senderDisplayName: String?, senderEmail: String?, senderID: String, messageContent: EmoteMessageContent) -> EmoteRoomTimelineItemContent {
        // PG_CHANGED
        let name = [senderDisplayName, senderEmail].compactMap { $0 }.first { !$0.isEmpty } ?? senderID
    ...
    private func buildStateProfileChangeTimelineItem(for eventItemProxy: EventTimelineItemProxy,
        ...
        guard let text = stateEventStringBuilder.buildProfileChangeString(displayName: displayName,
                                                                          ...
                                                                          // PG_CHANGED - render email instead of mxid
                                                                          member: eventItemProxy.sender.emailOrId,
    ...
    // PG_CHANGED
    private func buildMessageTimelineItemContent(messageType: MessageType?, senderID: String, senderDisplayName: String?, senderEmail: String?) -> EventBasedMessageTimelineItemContentType {
        switch messageType {
        ...
        case .emote(let content):
            // PG_CHANGED
            .emote(buildEmoteTimelineItemContent(senderDisplayName: senderDisplayName, senderEmail: senderEmail, senderID: senderID, messageContent: content))
