...
class LiveLocationManager: NSObject, LiveLocationManagerProtocol, CLLocationManagerDelegate {
    ...
    init(clientProxy: ClientProxyProtocol,
        ...
        // PG_CHANGED - live location sharing is off by default and only reachable through Developer Options,
        // so the shipping app no longer declares the `location` background mode (App Store review rejects it
        // without a user facing feature that needs persistent location). Setting allowsBackgroundLocationUpdates
        // without that background mode raises an exception, so only opt in when the target declares it.
