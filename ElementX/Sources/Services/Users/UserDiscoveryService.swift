...
final class UserDiscoveryService: UserDiscoveryServiceProtocol {
    ...
    // PG_CHANGED - falls back to a local profile when the remote lookup fails.
    /// Get remote profile or fallback to a "local profile" if the profile api fails
    private func getRemoteProfileOrLocal(userId: String, email: String? = nil) async -> UserProfileProxy {
        await (try? clientProxy.profile(for: userId).get()) ?? .init(userID: userId, email: email)
    }
    
    private func profileIfPossible(with searchQuery: String) async -> UserProfileProxy? {
        guard searchQuery.isMatrixIdentifier, searchQuery != clientProxy.userID else { return nil }
        // PG_CHANGED - uses the remote-or-local fallback helper for direct profile lookups.
        return await getRemoteProfileOrLocal(userId: searchQuery)
    }
