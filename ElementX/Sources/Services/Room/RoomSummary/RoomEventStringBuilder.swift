...
struct RoomEventStringBuilder {
    ...
    func buildAttributedString(for content: TimelineItemContent, sender: TimelineItemSender, isOutgoing: Bool) -> AttributedString? {
        // PG_CHANGED - fall back to the sender's email or ID instead of just the ID
        let displayName = sender.disambiguatedDisplayName ?? sender.emailOrId
        ...
        // PG_CHANGED
        case .roomMembership(let userID, let displayName, let email, let change, let reason):
            return stateEventStringBuilder
                // PG_CHANGED
                .buildString(for: change, reason: reason, memberUserID: userID, memberDisplayName: displayName, memberEmail: email, sender: sender, isOutgoing: isOutgoing)
                .map(AttributedString.init)
