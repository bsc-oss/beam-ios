...
extension RoomSummary: CustomStringConvertible {
    ...
    var roomListDescription: String {
        if isDirect {
            // PG_CHANGED - displays email for DM rooms in message forwarding, global search and share destination list
            return heroes.first?.emailOrId ?? canonicalAlias ?? ""
        }
        ...
        // PG_CHANGED
        var heroComponents = heroes.compactMap { $0.displayName ?? $0.email }
