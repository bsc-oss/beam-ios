...
class ClientProxy: ClientProxyProtocol {
    ...
    private let userProfileProxySubject = CurrentValueSubject<UserProfileProxy?, Never>(nil)
    var userProfileProxyPublisher: CurrentValuePublisher<UserProfileProxy?, Never> {
        userProfileProxySubject.asCurrentValuePublisher()
    }
    ...
    init(client: ClientProtocol,
        ...
        delegateHandle = try client.setDelegate(delegate: ClientDelegateWrapper { [weak self] isSoftLogout in
            ...
            case .panic(let message, let backtrace):
                ...
                // PG_CHANGED - adapts condition with redefined appBuildType
                if AppSettings.appBuildType.configuration == .debug || [.dev, .tst].contains(AppSettings.appBuildType.scheme) {
                    fatalError(message ?? "")
                }
    ...
    func stopSync(completion: (() -> Void)?) {
        MXLog.info("PG_CHANGED - Stopping sync - start")
        ...
        Task { [syncService] in
            defer {
                MXLog.info("PG_CHANGED - Stopping sync - calling completion handler")
                completion?()
            }
            
            MXLog.info("PG_CHANGED - Stopping sync - calling syncService.stop()")
            await syncService.stop()
            MXLog.info("PG_CHANGED - Sync stopped - syncService.stop() returned")
    ...
    func uploadArgusFile(_ media: MediaInfo, fileName: String?, fields: [String: String]) async -> Result<String, ClientProxyError> {
        ...
        } else if !media.url.lastPathComponent.isEmpty {
            resolvedFileName = media.url.lastPathComponent
        } else {
            resolvedFileName = "upload.bin"
        }

        do {
            let data = try Data(contentsOf: media.url)
            let fileId = try await client.uploadArgusFile(mimeType: mimeType,
                                                          filename: resolvedFileName,
                                                          data: data,
                                                          fields: fields.isEmpty ? nil : fields)
            return .success(fileId)
        } catch {
            MXLog.error("Failed uploading Argus media with error: \(error)")
            return .failure(ClientProxyError.sdkError(error))
        }
    }
    ...
    func reportSighting(_ request: CreateSightingRequest) async -> Result<Void, ClientProxyError> {
        do {
            try await client.reportSighting(request: request)
            return .success(())
        } catch {
            MXLog.error("Failed reporting sighting with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func loadUserProfileProxy() async -> Result<Void, ClientProxyError> {
        MXLog.info("PG_CHANGED - loadUserProfileProxy - start")
        do {
            let userProfileProxy = try await profile(for: userID).get()
            userProfileProxySubject.send(userProfileProxy)
            MXLog.info("PG_CHANGED - loadUserProfileProxy - completed successfully")
            return .success(())
        } catch {
            MXLog.error("Failed loading user profile proxy with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func setUserDisplayName(_ name: String) async -> Result<Void, ClientProxyError> {
        do {
            MXLog.info("PG_CHANGED - setUserDisplayName - calling client.setDisplayName")
            try await client.setDisplayName(name: name)
            MXLog.info("PG_CHANGED - setUserDisplayName - client.setDisplayName completed")
            Task {
                // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
                MXLog.info("PG_CHANGED - setUserDisplayName - fire-and-forget: starting loadUserProfileProxy")
                _ = await self.loadUserProfileProxy()
                MXLog.info("PG_CHANGED - setUserDisplayName - fire-and-forget: loadUserProfileProxy completed")
            }
            return .success(())
        } catch {
    ...
    func setUserFunction(_ function: String) async -> Result<Void, ClientProxyError> {
        do {
            MXLog.info("PG_CHANGED - setUserFunction - calling client.setFunction")
            try await client.setFunction(function: function)
            MXLog.info("PG_CHANGED - setUserFunction - client.setFunction completed")
            Task {
                MXLog.info("PG_CHANGED - setUserFunction - fire-and-forget: starting loadUserProfileProxy")
                _ = await self.loadUserProfileProxy()
                MXLog.info("PG_CHANGED - setUserFunction - fire-and-forget: loadUserProfileProxy completed")
            }
            return .success(())
        } catch {
            MXLog.error("Failed setting user function with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    
    // PG_CHANGED: replaced setPhoneBookConsent(Bool) with setPhoneBookConsentType(PhoneBookConsentType)
    func setPhoneBookConsentType(_ type: PhoneBookConsentType) async -> Result<Void, ClientProxyError> {
        do {
            MXLog.info("PG_CHANGED - setPhoneBookConsentType - calling client.setPhoneBookConsent")
            try await client.setPhoneBookConsent(consentType: type.ffiConsent)
            MXLog.info("PG_CHANGED - setPhoneBookConsentType - client.setPhoneBookConsent completed")
            Task {
                MXLog.info("PG_CHANGED - setPhoneBookConsentType - fire-and-forget: starting loadUserProfileProxy")
                _ = await self.loadUserProfileProxy()
                MXLog.info("PG_CHANGED - setPhoneBookConsentType - fire-and-forget: loadUserProfileProxy completed")
            }
            return .success(())
        } catch {
            MXLog.error("Failed setting phone book consent type with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func setProfileInitialized(_ isInitialized: Bool) async -> Result<Void, ClientProxyError> {
        do {
            try await client.setProfileInitialized(isInitialized: isInitialized)
            return .success(())
        } catch {
            MXLog.error("Failed setting profile initialized with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func setUserAvatar(media: MediaInfo) async -> Result<Void, ClientProxyError> {
        ...
        do {
            ...
            MXLog.info("PG_CHANGED - setUserAvatar - calling client.uploadAvatar")
            try await client.uploadAvatar(mimeType: mimeType, data: data)
            MXLog.info("PG_CHANGED - setUserAvatar - client.uploadAvatar completed")
            Task {
                // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
                MXLog.info("PG_CHANGED - setUserAvatar - fire-and-forget: starting loadUserProfileProxy")
                _ = await self.loadUserProfileProxy()
                MXLog.info("PG_CHANGED - setUserAvatar - fire-and-forget: loadUserProfileProxy completed")
            }
            return .success(())
        } catch {
    ...
    func removeUserAvatar() async -> Result<Void, ClientProxyError> {
        do {
            MXLog.info("PG_CHANGED - removeUserAvatar - calling client.removeAvatar")
            try await client.removeAvatar()
            MXLog.info("PG_CHANGED - removeUserAvatar - client.removeAvatar completed")
            Task {
                // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
                MXLog.info("PG_CHANGED - removeUserAvatar - fire-and-forget: starting loadUserProfileProxy")
                _ = await self.loadUserProfileProxy()
                MXLog.info("PG_CHANGED - removeUserAvatar - fire-and-forget: loadUserProfileProxy completed")
            }
            return .success(())
        } catch {
    ...
    func recentConversationCounterparts() async -> [UserProfileProxy] {
        ...
        for roomID in roomIdentifiers {
            ...
            for member in members where member.isActive && member.userID != userID {
                // PG_CHANGED
                users.append(.init(userID: member.userID, displayName: member.displayName, avatarURL: member.avatarURL, email: member.email))
    ...
    private func loadUserAvatarURLFromCache() {
        loadCachedAvatarURLTask = Task {
            do {
                ...
                // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
                if let previousValue = self.userProfileProxySubject.value {
                    self.userProfileProxySubject.value = UserProfileProxy(userID: previousValue.userID,
                                                                          displayName: previousValue.displayName,
                                                                          avatarURL: urlString.flatMap(URL.init),
                                                                          email: previousValue.email,
                                                                          // PG_CHANGED - adds custom profile fields
                                                                          department: previousValue.department,
                                                                          function: previousValue.function,
                                                                          phoneBookConsentType: previousValue.phoneBookConsentType,
                                                                          isProfileInitialized: previousValue.isProfileInitialized,
                                                                          featureFlags: previousValue.featureFlags)
                } else if let cachedAvatarURL = urlString.flatMap(URL.init) {
                    // PG_CHANGED - only create a minimal profile stub when there's actually a
                    // cached avatar URL to show. Creating a stub with all-nil fields would cause
                    // ensureUserProfileLoaded() to skip the network fetch, breaking onboarding checks.
                    self.userProfileProxySubject.value = UserProfileProxy(userID: self.userID,
    ...
    func reciprocateWithQrCode(qrCodeData: QrCodeData, progressListener: QrLoginProgressListener) async -> Result<Void, ClientProxyError> {
        do {
            try await client.reciprocateWithQrCode(qrCodeData: qrCodeData, progressListener: progressListener)
            return .success(())
        } catch {
            MXLog.error("Failed reciprocating with QR code with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func resumeQrReciprocation() async -> Result<Void, ClientProxyError> {
        do {
            try await client.resumeQrReciprocation()
            return .success(())
        } catch {
            MXLog.error("Failed resuming QR reciprocation with error: \(error)")
            return .failure(.sdkError(error))
        }
    }
}
