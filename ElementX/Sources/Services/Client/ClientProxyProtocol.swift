...
protocol ClientProxyProtocol: AnyObject {
    ...
    var userProfileProxyPublisher: CurrentValuePublisher<UserProfileProxy?, Never> { get }
    ...
    // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
    @discardableResult func loadUserProfileProxy() async -> Result<Void, ClientProxyError>
    ...
    // PG_CHANGED
    /// Reciprocate a QR login by scanning a Login QR from a new device.
    /// Forwards progress updates to the provided listener.
    ...
    /// Resume a previously started QR reciprocation after returning from the browser.
