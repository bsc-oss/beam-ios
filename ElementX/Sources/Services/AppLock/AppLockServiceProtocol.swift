...
protocol AppLockServiceProtocol: AnyObject {
    ...
    /// PG_CHANGED: Whether the device has a secure lock screen (passcode, Face ID, or Touch ID) configured at the OS level.
    var deviceHasLockScreen: Bool { get }
...
extension AppLockServiceMock {
    /// PG_CHANGED
    static func mock(pinCode: String? = "2023", isMandatory: Bool = false, deviceHasLockScreen: Bool = true, biometryType: LABiometryType = .faceID, numberOfPINAttempts: Int = 0) -> AppLockServiceMock {
        let mock = AppLockServiceMock()
        mock.isEnabled = pinCode != nil
        mock.isMandatory = isMandatory
        /// PG_CHANGED
        mock.deviceHasLockScreen = deviceHasLockScreen
