...
enum QRLoginProgress {
    ...
    /// PG_CHANGED: add confirmingLogin case for reciprocation flow
    case confirmingLogin(verificationUri: String, verificationUriComplete: String?)
    case syncingSecrets
    ...
    init?(rustProgress: QrLoginProgress) {
        ...
        // PG_CHANGED: map confirmingLogin case
        case .confirmingLogin(verificationUri: let verificationUri, verificationUriComplete: let verificationUriComplete):
            self = .confirmingLogin(verificationUri: verificationUri, verificationUriComplete: verificationUriComplete)
        }
    }
}
...
extension QRLoginProgress: Equatable, CustomStringConvertible {
    static func == (lhs: QRLoginProgress, rhs: QRLoginProgress) -> Bool {
        ...
        case (.signedIn, .signedIn):
            true
        // PG_CHANGED
        case let (.confirmingLogin(lhsUri, lhsComplete), .confirmingLogin(rhsUri, rhsComplete)):
            lhsUri == rhsUri && lhsComplete == rhsComplete
        default:
            false
        }
    }
    ...
    var description: String {
        ...
        case .signedIn: "signedIn"
        // PG_CHANGED
        case .confirmingLogin: "confirmingLogin"
        }
    }
}

// PG_CHANGED
extension QRCodeLoginServiceProtocol {
    func resumeReciprocation() async { }
}
