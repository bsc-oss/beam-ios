...
extension ClientProxyMock {
    convenience init(_ configuration: ClientProxyMockConfiguration) {
        ...
        userProfileProxyPublisher = .init(UserProfileProxy(userID: userID, displayName: "User display name", avatarURL: nil, phoneBookConsentType: .full))
        ...
        uploadArgusFileFileNameFieldsReturnValue = .failure(.sdkError(ClientProxyMockError.generic))
        reportSightingReturnValue = .failure(.sdkError(ClientProxyMockError.generic))
        // PG_CHANGED - replaces /profile/{id}/displayname|avatar_url endpoints by /profile/{id}
        loadUserProfileProxyReturnValue = .failure(.sdkError(ClientProxyMockError.generic))
        ...
        // PG_CHANGED - replaced setPhoneBookConsent with setPhoneBookConsentType
        setPhoneBookConsentTypeReturnValue = .success(())
