...
struct NotificationContentBuilder {
    ...
    private func processInvited(notificationContent: inout UNMutableNotificationContent,
        ...
        // PG_CHANGED - displays user email in notification for invites
        let name = if let senderDisplayName = notificationItem.senderDisplayName {
            "\(senderDisplayName) (\(notificationItem.senderEmail ?? notificationItem.senderID))"
        } else {
