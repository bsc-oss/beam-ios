...
public struct CompoundButtonStyle: ButtonStyle {
    private static let baseBackgroundButton = RoundedRectangle(cornerRadius: 8)
    ...
    private func makeBackground(configuration: Self.Configuration) -> some View {
        ...
        case .super:
            if isEnabled {
                ZStack {
                    CompoundButtonStyle.baseBackgroundButton.fill(.compound.bgCanvasDefault)
                    CompoundButtonStyle.baseBackgroundButton.fill(LinearGradient(gradient: .compound.action,
                                                                                 startPoint: .top, endPoint: .bottom))
                        .opacity(0.04)
                    CompoundButtonStyle.baseBackgroundButton.strokeBorder(LinearGradient(gradient: .compound.action,
                                                                                         startPoint: .top, endPoint: .bottom))
                }
                .compositingGroup()
                .opacity(configuration.isPressed ? pressedOpacity : 1)
            } else {
                CompoundButtonStyle.baseBackgroundButton.strokeBorder(strokeColor(configuration: configuration))
            }
        case .primary:
            CompoundButtonStyle.baseBackgroundButton.fill(fillColor(configuration: configuration))
        case .secondary:
            CompoundButtonStyle.baseBackgroundButton.strokeBorder(strokeColor(configuration: configuration))
        case .tertiary:
            EmptyView()
    ...
    private var contentShape: AnyShape {
        ...
        case .super, .primary, .secondary, .tertiary:
            return AnyShape(CompoundButtonStyle.baseBackgroundButton)
        case .textLink:
